...
async function migrateAllVersions(repositoryPath, targetDirectory, product = 'generic', options = {}) {
  ...
  if (fs.existsSync(sourceBase)) {
    ...
    for (const version of versions) {
      ...
      const result = await processDirectory(sourceDir, targetDir, formattedVersion, product);
  ...
  if (fs.existsSync(currentDocsPath)) {
    ...
    const result = await processDirectory(currentDocsPath, targetDir, 'next', product);
  ...
  // 4. Update navigation (only if explicitly requested)
  if (updateNavigation) {
