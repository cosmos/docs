...
export function renderModulePage(module, types, context, headings) {
  ...
  out.push('<Info>');
  out.push(
    `  Generated from [\`${context.repository}@${context.sha.slice(0, 12)}\`](https://github.com/${context.repository}/tree/${context.sha}/proto) on ref \`${context.ref}\`, documenting Cosmos SDK ${context.displayVersion}.`,
  );
  out.push('');
  out.push(
  ...
  if (queryCount) {
    ...
    out.push(
      `Examples are templates: replace every \`<string>\` placeholder with a real value, and drop optional filter fields you do not want to filter on. Payloads here are protobuf JSON, where a \`cosmos.Dec\` is the scaled integer string rather than a decimal. A \`string\` field may take an enum constant, listed under Types. Examples assume a local node; against a TLS endpoint drop \`-plaintext\` and use port 443. See [gRPC services](/sdk/${context.version}/api-reference/grpc/index) for reflection recipes, address formats, and how decimal values are encoded.`,
    );
    out.push('');
  ...
  if (msgCount) {
    ...
    out.push('');
    out.push(
      `These are not callable endpoints. Each one is encoded into a transaction, signed, and broadcast. See [Sending Transactions](/sdk/${context.version}/api-reference/transactions) for how to build, sign, and broadcast one. The handler name is shown for reference and for generated clients, not as an address you can call directly.`,
    );
    out.push('');
    out.push(
      `The JSON below is transaction JSON, the form the CLI writes and reads and the form \`POST /cosmos/tx/v1beta1/encode\` accepts. A \`cosmos.Dec\` field there is a plain decimal string such as \`"0.05"\`. This holds even where the field's proto type is \`bytes\`: the base64 form is how such a field reads back over gRPC, not how it is written. The same field passed to the \`cosmos.tx.v1beta1.Service/TxEncode\` gRPC method instead needs the scaled integer string \`"50000000000000000"\`, because that path uses a different codec. See [scalar encodings](/sdk/${context.version}/api-reference/grpc/index#scalar-encodings).`,
    );
    out.push('');
    out.push(
