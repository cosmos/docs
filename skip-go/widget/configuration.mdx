...
Customizes the initial route displayed on widget load. Query supported assets using the Skip Go API [/assets](/skip-go/api-reference/prod/fungible/get-v2fungibleassets) endpoint. Setting this triggers a route request on render.
...
- `allowUnsafe`: Allow unsafe routes. Default: false. [More info](/skip-go/advanced-swapping/allow_unsafe-preventing-handling-bad-execution).
...
- `goFast`: Enable Go Fast transfers. Default: false. [More info](/skip-go/advanced-transfer/go-fast).
- `smartSwapOptions`: Advanced swapping features like EVM Swaps and split trade routes. [More info](/skip-go/advanced-swapping/smart-swap-options).
