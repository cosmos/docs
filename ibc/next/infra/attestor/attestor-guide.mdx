...
- **Remote signer** — the attestor delegates signing to an external gRPC signer service over the network
...
For remote signers, retrieve the signing key's Ethereum address from your signer service before creating the light client. The remote signer must implement the [gRPC interface the attestor expects](https://github.com/cosmos/ibc-relayer#remote-signing).
