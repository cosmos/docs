...
The client trusts a fixed set of Ethereum-style ECDSA addresses (secp256k1 EOAs) configured at creation time. Proof verification requires at least `minRequiredSigs` unique valid signatures from that set. The attestor set cannot be changed after creation.
