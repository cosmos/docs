...
To supplement this, a [sentinel `ConnectionEnd` is stored in core IBC](/ibc/next/light-clients/localhost/connection) state with the connection
...
[State verification](/ibc/next/light-clients/localhost/state-verification) for channel state in handshakes or processing packets is reduced in
