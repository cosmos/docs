...
message MsgSendCall {
  ...
  uint64 timeout_timestamp = 6; // absolute timeout in Unix seconds
...
address accountAddress = Create2.deploy(0, accountIdHash, beaconProxyBytecode);
```
...
The account address is a deterministic 32-byte module address ([`account.go`](https://github.com/cosmos/ibc-go/blob/main/modules/apps/27-gmp/types/account.go)):

```go
func BuildAddressPredictable(accountID *AccountIdentifier) (sdk.AccAddress, error) {
    key := len(clientId) || clientId || len(sender) || sender || len(salt) || salt
    return address.Module("gmp-accounts", key)[:32]
}
```
...
The [IBC demo](https://github.com/cosmos/ibc-e2e-docs-example) uses GMP for both directions of IFT token transfers. 
