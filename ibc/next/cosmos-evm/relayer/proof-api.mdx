...
When the relayer identifies a packet that needs relaying, it calls the Proof API's `RelayByTx` RPC with the source transaction hash, source and destination chain IDs, and the relevant IBC client IDs. The Proof API:
...
4. Constructs the full relay transaction — `updateClient` + `RecvPacket` (or `AckPacket` / `Timeout`) — with the attestation proofs embedded
