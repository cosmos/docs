...
Each attestor instance handles one chain type. For m-of-n quorum, multiple instances per chain can be deployed with distinct signing keys. All of their addresses must be registered with the light client. The quorum threshold set on the light client determines how many must sign for a packet to be accepted.

{/* todo: link above to light client docs */}
