...
For conceptual background on how the system works before diving in, see the [architecture overview](../overview.mdx).

{/* todo: update link above */}

## Getting Started

This demo has a quickstart that runs everything in a single command (`./setup.sh ibc`). After getting set up, you can run each step individually using the walkthrough sections.
...
4. Create attestation light clients. Each chain needs a light client that can verify packets arriving from the other side. Both are initialized with the attestor's signing address, so the attestors must be running before this step.

5. Configure and start the Proof API and relayer. The Proof API aggregates attestor signatures into relay-ready proofs. The relayer uses those proofs to submit packet delivery and timeout transactions on both chains. 
