...
The demo uses local signing via `keys.json`:

```json
{
  "32382":    {"private_key": "ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"},
  "cosmos-1": {"private_key": "7d4a0a3c3dfa9b5749958493b6aefba761f1c59f8e553a5f32e1556c15f23e57"}
}
```

For production, the relayer supports a remote gRPC signing service that keeps private keys isolated from the relayer process. Replace `keys_path` in the `signing` block of `config.yml` with:
