...
Two additional contracts are deployed in later steps: `AttestationLightClient` in the [`create-clients` step](/ibc/next/cosmos-solidity/tutorial/walkthrough/05-clients) once the attestor address is known, and `CosmosIFTSendCallConstructor` in `wire` once the client-derived account address is derived.
...
If you are operating a Cosmos EVM chain and need to access IBC through the EVM, you can deploy the same solidity contracts in this guide to your Cosmos EVM chain. This is useful if you need to access IBC through the EVM on a Cosmos EVM chain.
</Tip>
...
{/* dennis serdar todo: Please verify that this list and the one below are complete and add any more commentary on contract deployment. */}
...
A complete list of the IBC Solidity contracts, see the [cosmos/solidity-ibc-eureka](https://github.com/cosmos/solidity-ibc-eureka) repository.
...
{/* dennis serdar todo: Please fill in this section with considerations on deploying solditiy contracts on a cosmos EVM chain. What is different? what should be considered? how does it make the other steps after this different?  */}
...
{/* dennis serdar todo: Please verify and add any more commentary on contract deployment. Can the deploy scripts in this demo be reused? */}

### Access control

The demo uses the deployer address as both the AccessManager admin and the relayer EOA. For production, you may want to grant roles explicitly:
...
{/* dennis serdar todo: I know there are more options here. Please provide a set of recommendations for the permission levels of the various restricted functions for production use. */}
...
{/* dennis serdar todo: Please add in recommendations here for running the IFT contract in production/which implementation to use.  */}
