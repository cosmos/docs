...
The attestor address must exist before the [`create-clients` step](/ibc/next/cosmos-solidity/tutorial/walkthrough/05-clients) because it is baked into both light clients at creation time.
...
This address is what gets registered with both light clients in the [`create-clients` step](/ibc/next/cosmos-solidity/tutorial/walkthrough/05-clients). In this demo, the same keystore is mounted into both attestor containers so both sign with the same address.
