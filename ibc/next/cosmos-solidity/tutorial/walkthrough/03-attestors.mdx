...
The attestor address must exist before the [`create-clients` step](/ibc/next/cosmos-solidity/tutorial/walkthrough/04-clients) because it is baked into both light clients at creation time.
...
This address is what gets registered with both light clients in the [`create-clients` step](/ibc/next/cosmos-solidity/tutorial/walkthrough/04-clients). In this demo, the same keystore is mounted into both attestor containers so both sign with the same address.
...
<Note>
  `adapter.finality_offset` determines at which block height the attestor reads and signs chain state. If omitted, the attestor uses the chain's native `finalized` block tag. If set to `N`, it attests `latest - N`. This value must match `finality_offset` in the relayer config. See [Finality offset](./04-relayer#finality-offset) for more details.
</Note>
...
| `adapter.finality_offset` | No | Blocks to subtract from latest when determining finality — must match the relayer's `finality_offset` (see [Finality offset](./04-relayer#finality-offset)) |
...
The next step creates the attestation light clients on both chains. Both clients are initialized with the attestor address generated in this step, so the keystore must exist before proceeding.
