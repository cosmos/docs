...
This value should match the finality assumption your attestor is configured with. If the attestor attests state at a block the relayer does not yet consider final, proofs will be fetched but the relay will not proceed until finality catches up.
...
With the relayer and Proof API running, the next step wires the connection: registering each client as the counterparty of the other and linking the IFT token contracts on both chains.
