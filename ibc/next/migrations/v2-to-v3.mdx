...
Please see the [ICS27 documentation](/ibc/next/apps/interchain-accounts/overview) for more information.
...
For ICS27 it is also necessary to [manually add store upgrades](/sdk/latest/guides/upgrades/upgrade#adding-new-modules-during-an-upgrade) for the new ICS27 module and then configure the store loader to apply those upgrades in `app.go`:
