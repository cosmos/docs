...
Applications can also be layered: [middleware](/ibc/next/middleware/overview) wraps an existing application to add behavior such as fee handling or packet forwarding, without modifying the underlying application. One example of an IBC application is the GMP (General Message Passing) module, which enables arbitrary cross-chain message execution. Visit the [GMP module](/ibc/next/modules/gmp/overview) page for a detailed overview, or try the [tutorial](/ibc/next/cosmos-evm/tutorial/introduction) for a hands-on demo. To learn how to build your own IBC application, see the [IBC v2 Applications](/ibc/next/apps/overview) page.