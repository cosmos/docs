...
<Warning>
  Initiating an upgrade in the same block as opening a channel may potentially
  prevent the counterparty channel from also opening.
</Warning>
...
after which the packet or upgrade times out
  Timestamp uint64
}
```
