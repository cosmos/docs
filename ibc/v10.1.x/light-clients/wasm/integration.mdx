---
noindex: true
canonical: 'https://docs.cosmos.network/ibc/latest/light-clients/wasm/integration'
title: Integration
description: >-
  Learn how to integrate the 08-wasm module in a chain binary and about the
  recommended approaches depending on whether the x/wasm module is already used
  in the chain. The following document only applies for Cosmos SDK chains.
---

Learn how to integrate the `08-wasm` module in a chain binary and about the recommended approaches depending on whether the [`x/wasm` module](https://github.com/CosmWasm/wasmd/tree/main/x/wasm) is already used in the chain. The following document only applies to Cosmos SDK chains.
...
The sample code below shows the relevant integration points in `app.go` required to set up the `08-wasm` module in a chain binary. Since `08-wasm` is a light client module itself, please also check out the section [Integrating light clients](/ibc/v10.1.x/ibc/integration#integrating-light-clients) for more information:
...
Or alternatively the parameter can be updated via a governance proposal (see at the bottom of section [`Creating clients`](/ibc/v10.1.x/light-clients/developer-guide/setup#creating-clients) for an example of how to do this).
