---
noindex: true
canonical: 'https://docs.cosmos.network/ibc/latest/light-clients/solomachine/concepts'
title: Concepts
description: >-
  The ClientState for a solo machine light client stores the latest sequence,
  the frozen sequence, the latest consensus state, and client flag indicating if
  the client should be allowed to be updated after a governance proposal.
---
...
The consensus state stores the public key, diversifier, and timestamp of the solo machine light client.
...
A solo machine proof should verify that the solo machine public key signed
...
* the header provided is parseable as a solo machine header
...
* the substitute provided is parseable as a solo machine client state
...
* the misbehaviour provided is parseable as a solo machine misbehaviour
