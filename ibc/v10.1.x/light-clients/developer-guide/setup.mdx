---
noindex: true
canonical: 'https://docs.cosmos.network/ibc/latest/light-clients/developer-guide/setup'
title: Setup
---
...
The last step to finish the development of the light client is to implement the `AppModuleBasic` interface to allow it to be added to the chain's `app.go` alongside other light client types the chain enables.
...
Please refer to the [core IBC documentation](/ibc/v10.1.x/ibc/integration#integrating-light-clients) for how to configure additional light client modules alongside `07-tendermint` in `app.go`.
