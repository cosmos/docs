...
To supplement this, a [sentinel `ConnectionEnd` is stored in core IBC](/ibc/v10.1.x/light-clients/localhost/connection) state with the connection
...
[State verification](/ibc/v10.1.x/light-clients/localhost/state-verification) for channel state in handshakes or processing packets is reduced in complexity; the `09-localhost` client can simply compare bytes stored under the standardized key paths.
