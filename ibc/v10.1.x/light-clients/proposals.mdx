---
noindex: true
canonical: 'https://docs.cosmos.network/ibc/latest/light-clients/proposals'
...
If one-third of the validator set of the chain the client represents decides to collude,
they can sign off on two valid but conflicting headers each signed by another one-third
...
See also the relevant documentation: ADR-026, IBC client recovery mechanisms
...
If the chain is on an ibc-go version older than v8, please see the [relevant documentation](/ibc/v7.8.x/ibc/proposals).
