---
noindex: true
canonical: 'https://docs.cosmos.network/ibc/latest/apps/transfer/authorizations'
title: Authorizations
---

`TransferAuthorization` implements the `Authorization` interface for `ibc.applications.transfer.v1.MsgTransfer`. It allows a granter to grant a grantee the privilege to submit `MsgTransfer` on its behalf. Please see the [Cosmos SDK docs](/sdk/v0.53/build/modules/authz) for more details on granting privileges via the `x/authz` module.
