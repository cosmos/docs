---
noindex: true
canonical: 'https://docs.cosmos.network/hub/latest/index'
title: Introduction
description: 'Welcome to the Cosmos Hub'
---

The Cosmos Hub is the first of many [interconnected blockchains](https://cosmos.network) powered by the interchain stack: [CometBFT](https://cometbft.com), [CosmosSDK](https://docs.cosmos.network) and [IBC](/ibc/next/intro). The primary token of the Cosmos Hub is the **ATOM**.
...
Learn more about [being a delegator](/hub/v25/delegators/delegator-faq), learn about [the security risks](/hub/v25/delegators/delegator-security), and start participating with one of the following wallets.
...
* [Komodo Wallet](https://komodoplatform.com/en/wallets)
...
`gaiad` is a command-line interface that lets you interact with the Cosmos Hub. `gaiad` is the only tool that supports 100% of the Cosmos Hub features, including accounts, transfers, delegation, and governance. Learn more about `gaiad` with the [delegator's CLI guide](/hub/v25/delegators/delegator-guide-cli).

## Running a full-node on the Cosmos Hub Mainnet

In order to run a full-node for the Cosmos Hub mainnet, you must first [install `gaiad`](/hub/v25/getting-started/installation). Then, follow [the guide](/hub/v25/hub-tutorials/join-mainnet).
If you are looking to run a validator node, follow the [validator setup guide](/hub/v25/validators/validator-setup).
