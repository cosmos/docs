---
title: x/tokenfactory
description: >-
  The tokenfactory module used by the Hub allows any account to create a new token
  with the denom factory/{creator address}/{subdenom}.
---
The `tokenfactory` module used by the Hub allows any account to create a new token with the denom `factory/{creator address}/{subdenom}`.

Gaia uses a fork of Strangelove's implementation of the tokenfactory module: [cosmos/tokenfactory](https://github.com/cosmos/tokenfactory). The fork introduces one change with respect to the Strangelove implementation: the sudo mint capability has been removed.
