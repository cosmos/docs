...
* [Install Gaia](/hub/v26/getting-started/installation)
...
Cosmos uses [HD Wallets](/hub/v26/resources/hd-wallets). This means you can setup many accounts using the same Ledger seed. To create another account from your Ledger device, run (change the integer i to some value `>=` 0 to choose the account for HD derivation):
...
<Tip>
To run your own full node locally [read more here.](/hub/v26/hub-tutorials/join-mainnet).
</Tip>
...
Now, you are all set to start [sending transactions on the network](/hub/v26/delegators/delegator-guide-cli#sending-transactions).
