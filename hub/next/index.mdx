...
Learn more about [being a delegator](/hub/next/delegators/delegator-faq), learn about [the security risks](/hub/next/delegators/delegator-security), and start participating with one of the following wallets.
...
`gaiad` is a command-line interface that lets you interact with the Cosmos Hub. `gaiad` is the only tool that supports 100% of the Cosmos Hub features, including accounts, transfers, delegation, and governance. Learn more about `gaiad` with the [delegator's CLI guide](/hub/next/delegators/delegator-guide-cli).

## Running a full-node on the Cosmos Hub Mainnet

In order to run a full-node for the Cosmos Hub mainnet, you must first [install `gaiad`](/hub/next/getting-started/installation). Then, follow [the guide](/hub/next/hub-tutorials/join-mainnet).
If you are looking to run a validator node, follow the [validator setup guide](/hub/next/validators/validator-setup).
