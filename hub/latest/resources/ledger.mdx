...
* [Install Gaia](/hub/latest/getting-started/installation)
...
Cosmos uses [HD Wallets](/hub/latest/resources/hd-wallets). This means you can setup many accounts using the same Ledger seed. To create another account from your Ledger device, run (change the integer i to some value `>=` 0 to choose the account for HD derivation):
...
<Tip>
To run your own full node locally [read more here.](/hub/latest/hub-tutorials/join-mainnet).
</Tip>
...
Now, you are all set to start [sending transactions on the network](/hub/latest/delegators/delegator-guide-cli#sending-transactions).
