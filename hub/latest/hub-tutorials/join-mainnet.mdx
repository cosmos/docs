...
For instructions to bootstrap a node via Quicksync or State Sync, see the [Quickstart Guide](/hub/latest/getting-started/quickstart)

For instructions to join as a validator, please also see the [Validator Guide](/hub/latest/validators/overview).
...
* Ensure Gaia is properly installed. See the [installation guide](/hub/latest/getting-started/installation) for a walk-through.
* Follow the [configuration guide](/hub/latest/hub-tutorials/join-mainnet#general-configuration) to initialize and prepare the node to sync with the network.
...
Blocksync is faster than traditional consensus and syncs the chain from genesis by downloading blocks and verifying against the merkle tree of validators. For more information see [CometBFT's Blocksync Docs](/cometbft/latest/docs/core/Using-CometBFT#block-sync)
...
State Sync is an efficient and fast way to bootstrap a new node, and it works by replaying larger chunks of application state directly rather than replaying individual blocks or consensus rounds. For more information, see [CometBFT's State Sync docs](/cometbft/latest/docs/core/state-sync).
