---
noindex: true
canonical: 'https://docs.cosmos.network/cometbft/latest/docs/app-dev/Indexing-Transactions'
...
details on `Events`, see the [ABCI][/cometbft/v0.39/spec/abci/Outline#events] documentation.
...
The CometBFT transaction and block event indexer indexes a few select reserved events
by default.
...
You can query for a paginated set of transactions by their events by calling the
...
Check out the [RPC API reference](/cometbft/v0.39/api-reference/rpc/index) for the `tx_search` endpoint
for more information on query syntax and other options.

## Subscribing to Transactions

Clients can subscribe to transactions with the given tags via WebSocket by providing
a query to the `/subscribe` RPC endpoint.
...
Check out the [RPC API documentation](/cometbft/v0.39/api-reference/rpc/index) for more information
...
and transactions whose event attributes matched within the height but could match
across different events at that height.
...
[abci-events]: /cometbft/v0.39/spec/abci/abci++_basic_concepts#events
