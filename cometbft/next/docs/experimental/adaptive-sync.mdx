...
<Warning>
  ...
  for your specific workload before enabling it on a mainnet chain.
</Warning>
...
With `adaptive_sync` enabled, consensus still works normally, but it can also ingest already 
available blocks from blocksync. This acts as a fallback path when a node is behind, allowing it to recover
