...
instructions](/cometbft/next/docs/guides/Install-CometBFT) for details.
...
transactions](/cometbft/next/docs/app-dev/Indexing-Transactions) for details.
...
Applications can use [state sync](/cometbft/next/docs/core/state-sync) to help nodes bootstrap quickly.
...
modules can be found [here](/cometbft/next/docs/core/how-to-read-logs#list-of-modules). If
...
Validators are supposed to set up [Sentry Node Architecture](/cometbft/next/docs/core/Validators)
...
**Under no condition should any of the [unsafe RPC endpoints](/cometbft/next/api-reference/rpc/index#endpoint-categories)
ever be exposed publicly.**

#### Endpoints Returning Multiple Entries

Endpoints returning multiple entries are limited by default to return 30
elements (100 max). See the [RPC Documentation](/cometbft/next/api-reference/rpc/index)
for more information.

## Debugging CometBFT

If you ever have to debug CometBFT, the first thing you should probably do is
check out the logs. See [How to read logs](/cometbft/next/docs/core/how-to-read-logs), where we
...
[Debugging](/cometbft/next/docs/tools/debugging) for the exact format.
...
[Metrics](/cometbft/next/docs/core/metrics).

The `cometbft debug dump` subcommand can be used to periodically dump useful
information into an archive. See [Debugging](/cometbft/next/docs/tools/debugging) for more
information.
