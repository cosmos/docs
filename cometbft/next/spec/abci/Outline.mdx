...
[CometBFT's expected behavior](/cometbft/next/spec/abci/CometBFTs-expected-behavior) for details on the possible
call sequences of these methods.

- [**InitChain:**](/cometbft/next/spec/abci/Methods#initchain) This method initializes the blockchain.
  CometBFT calls it once upon genesis.

- [**PrepareProposal:**](/cometbft/next/spec/abci/Methods#prepareproposal) It allows the block
...
- [**ProcessProposal:**](/cometbft/next/spec/abci/Methods#processproposal) It allows a validator to
...
- [**ExtendVote:**](/cometbft/next/spec/abci/Methods#extendvote) It allows applications to let their
...
- [**VerifyVoteExtension:**](/cometbft/next/spec/abci/Methods#verifyvoteextension) It allows
...
- [**FinalizeBlock:**](/cometbft/next/spec/abci/Methods#finalizeblock) It delivers a decided block to the
...
- [**Commit:**](/cometbft/next/spec/abci/Methods#commit) Instructs the Application to persist its
...
- [**CheckTx:**](/cometbft/next/spec/abci/Methods#checktx) This method allows the Application to validate
...
- [**Info:**](/cometbft/next/spec/abci/Methods#info) Used to sync CometBFT with the Application during a
  handshake that happens upon recovery, or on startup when state-sync is used.

- [**Query:**](/cometbft/next/spec/abci/Methods#query) This method can be used to query the Application for
...
[state sync documentation](/cometbft/next/spec/p2p/legacy-docs/messages/state-sync).
...
[snapshot data type](/cometbft/next/spec/abci/Methods#snapshot) for more details). After receiving a
...
[light client verification](/cometbft/next/spec/light-client/verification)).

In summary:

- [**ListSnapshots:**](/cometbft/next/spec/abci/Methods#listsnapshots) Used by nodes to discover available
  snapshots on peers.

- [**OfferSnapshot:**](/cometbft/next/spec/abci/Methods#offersnapshot) When a node receives a snapshot from a
  peer, CometBFT uses this method to offer the snapshot to the Application.

- [**LoadSnapshotChunk:**](/cometbft/next/spec/abci/Methods#loadsnapshotchunk) Used by CometBFT to retrieve
  snapshot chunks from the Application to send to peers.

- [**ApplySnapshotChunk:**](/cometbft/next/spec/abci/Methods#applysnapshotchunk) Used by CometBFT to hand
  snapshot chunks to the Application.

### Other methods

Additionally, there is a [**Flush**](/cometbft/next/spec/abci/Methods#flush) method that is called on every connection,
and an [**Echo**](/cometbft/next/spec/abci/Methods#echo) method that is used for debugging.

More details on managing state across connections can be found in the section on
[Managing Application State](/cometbft/next/spec/abci/Requirements-for-the-Application#managing-the-application-state-and-related-topics).
...
information can be found in either [data structures](/cometbft/next/spec/core/Data_structures)
or [accountability](/cometbft/next/spec/light-client/Accountability).
