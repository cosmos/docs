...
As mentioned in the [Basic Concepts](/cometbft/next/spec/abci/Outline) section, CometBFT
...
* Also for every round, if the local process is _not_ the proposer of the current round, CometBFT
  ...
  see [this section](/cometbft/next/spec/abci/Introduction#scenario-3) for an example.
...
* `PrepareProposal` must create a list of [transactions](/cometbft/next/spec/abci/Methods#prepareproposal)
...
If applications upgrade to ABCI 2.0, CometBFT internally ensures that the [application setup](/cometbft/next/spec/abci/Requirements-for-the-Application#application-configuration-required-to-switch-to-abci-2-0) is reflected in its operation.
