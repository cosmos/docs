---
noindex: true
canonical: 'https://docs.cosmos.network/cometbft/latest/docs/tools/debugging'
title: Debugging
order: 1
---
...
state, the node's status, the WAL, and even the stack trace of the process
...
The `debug dump` sub-command allows you to dump debugging data into
...
for debugging.
Calling the `/status`, `/consensus_state`, and `/dump_consensus_state` RPC endpoints
...
Additional information on the CometBFT RPC endpoints can be found in the [RPC documentation](/cometbft/v0.38/api-reference/rpc/index).
