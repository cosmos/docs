---
noindex: true
canonical: 'https://docs.cosmos.network/cometbft/latest/docs/core/Using-CometBFT'
...
> TIP: Find the RPC Documentation [here](/cometbft/v0.38/api-reference/rpc/index)
...
config specification](/cometbft/v0.38/docs/core/configuration).
...
will be included in the mempool, broadcast to other peers, and
...
CheckTx (i.e., was accepted into the mempool) doesn't mean it will be committed,
as nodes with the tx in their mempool may crash before they get to propose.
For more information, see the [mempool
write-ahead-log](/cometbft/v0.38/docs/core/Running-in-production#mempool-wal).
...
recommend having voting powers greater than 10\^12 (i.e., 1 trillion).
...
Basically the seed node's job is just to relay everyone's addresses. You won't
...
If you want CometBFT to connect to a specific set of addresses and
...
stopping the CometBFT instance.
...
`priv_validator_key.json` and copy its `pub_key` into the above genesis.
...
developers guide](/cometbft/v0.38/docs/app-dev/Using-ABCI-CLI) for more details.
