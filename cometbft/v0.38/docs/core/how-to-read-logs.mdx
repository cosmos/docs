...
- `abci-client` As mentioned in [Application Development Guide](/cometbft/v0.38/docs/app-dev/Using-ABCI-CLI), CometBFT acts as an ABCI
...
- `events` Simple event notification system. The list of events can be
  ...
  You can subscribe to them by calling the `subscribe` RPC method. Refer
  to [RPC docs](/cometbft/v0.38/rpc) for additional information.
- `mempool` Mempool module handles all incoming transactions, whether
  they are coming from peers or the application.
- `p2p` Provides an abstraction around peer-to-peer communication. For
  more details, please check out the
  [README](https://github.com/cometbft/cometbft/blob/v0.38.x/p2p/README.md).
- `rpc` [CometBFT's RPC](/cometbft/v0.38/rpc).
