---
noindex: true
canonical: 'https://docs.cosmos.network/cometbft/latest/docs/core/state-sync'
title: State Sync
order: 11
---

With block sync, a node downloads all of the application's data from genesis and verifies it.
...
- `temp_dir`: Temporary directory to store the chunks in the machine's local storage. If nothing is set, it will create a directory in `/tmp`.

The next information you will need to acquire through publicly exposed RPCs or a block explorer which you trust.
...
If you are relying on publicly exposed RPCs to get the needed information, you can use `curl` and [`jq`][jq].
