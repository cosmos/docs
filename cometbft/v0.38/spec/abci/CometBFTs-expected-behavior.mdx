---
noindex: true
canonical: 'https://docs.cosmos.network/cometbft/latest/spec/abci/CometBFTs-expected-behavior'
order: 4
...
As mentioned in the [Basic Concepts](/cometbft/v0.38/spec/abci/Outline) section, CometBFT
...
* In _state-sync_ mode, CometBFT makes one or more attempts at synchronizing the Application's state.
  ...
  [CometBFT to ensure](https://github.com/cometbft/cometbft/blob/main/docs/references/rfc/rfc-100-abci-vote-extension-propag.md#base-implementation-persist-and-propagate-extended-commit-history)
...
* Also for every round, if the local process is _not_ the proposer of the current round, CometBFT
  ...
  see [this section](/cometbft/v0.38/spec/abci/Introduction#scenario-3) for an example.
...
* `PrepareProposal` must create a list of [transactions](/cometbft/v0.38/spec/abci/Methods#prepareproposal)
...
historical commits and potential optimizations, are discussed in detail in [RFC-100](https://github.com/cometbft/cometbft/blob/main/docs/references/rfc/rfc-100-abci-vote-extension-propag.md#current-limitations-and-possible-implementations)

## Handling upgrades to ABCI 2.0

If applications upgrade to ABCI 2.0, CometBFT internally ensures that the [application setup](/cometbft/v0.38/spec/abci/Requirements-for-the-Application#application-configuration-required-to-switch-to-abci-2-0) is reflected in its operation.
