...
[CometBFT's expected behavior](./abci++_comet_expected_behavior.mdx) for details on the possible
call sequences of these methods.

- [**InitChain:**](./abci++_methods.mdx#initchain) This method initializes the blockchain.
  CometBFT calls it once upon genesis.

- [**PrepareProposal:**](./abci++_methods.mdx#prepareproposal) It allows the block
...
- [**ProcessProposal:**](./abci++_methods.mdx#processproposal) It allows a validator to
...
- [**ExtendVote:**](./abci++_methods.mdx#extendvote) It allows applications to let their
...
- [**VerifyVoteExtension:**](./abci++_methods.mdx#verifyvoteextension) It allows
...
- [**FinalizeBlock:**](./abci++_methods.mdx#finalizeblock) It delivers a decided block to the
...
- [**Commit:**](./abci++_methods.mdx#commit) Instructs the Application to persist its
...
- [**CheckTx:**](./abci++_methods.mdx#checktx) This method allows the Application to validate
...
- [**Info:**](./abci++_methods.mdx#info) Used to sync CometBFT with the Application during a
  handshake that happens upon recovery, or on startup when state-sync is used.

- [**Query:**](./abci++_methods.mdx#query) This method can be used to query the Application for
...
[snapshot data type](./abci++_methods.mdx#snapshot) for more details). After receiving a
...
[light client verification](../light-client/verification/README.mdxx)).

In summary:

- [**ListSnapshots:**](./abci++_methods.mdx#listsnapshots) Used by nodes to discover available
  snapshots on peers.

- [**OfferSnapshot:**](./abci++_methods.mdx#offersnapshot) When a node receives a snapshot from a
  peer, CometBFT uses this method to offer the snapshot to the Application.

- [**LoadSnapshotChunk:**](./abci++_methods.mdx#loadsnapshotchunk) Used by CometBFT to retrieve
  snapshot chunks from the Application to send to peers.

- [**ApplySnapshotChunk:**](./abci++_methods.mdx#applysnapshotchunk) Used by CometBFT to hand
  snapshot chunks to the Application.

### Other methods

Additionally, there is a [**Flush**](./abci++_methods.mdx#flush) method that is called on every connection,
and an [**Echo**](./abci++_methods.mdx#echo) method that is used for debugging.

More details on managing state across connections can be found in the section on
[Managing Application State](./abci%2B%2B_app_requirements.mdx#managing-the-application-state-and-related-topics).
...
information can be found in either [data structures](../core/data_structures.mdxx)
