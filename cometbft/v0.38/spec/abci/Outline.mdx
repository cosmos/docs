...
- [**PrepareProposal:**](./Methods.mdx#prepareproposal) It allows the block
...
- [**ProcessProposal:**](./Methods.mdx#processproposal) It allows a validator to
...
- [**ExtendVote:**](./Methods.mdx#extendvote) It allows applications to let their
...
- [**VerifyVoteExtension:**](./Methods.mdx#verifyvoteextension) It allows
...
- [**FinalizeBlock:**](./Methods.mdx#finalizeblock) It delivers a decided block to the
...
- [**Commit:**](./Methods.mdx#commit) Instructs the Application to persist its
...
- [**CheckTx:**](./Methods.mdx#checktx) This method allows the Application to validate
...
- [**Info:**](./Methods.mdx#info) Used to sync CometBFT with the Application during a
  handshake that happens upon recovery, or on startup when state-sync is used.

- [**Query:**](./Methods.mdx#query) This method can be used to query the Application for
...
[snapshot data type](./Methods.mdx#snapshot) for more details). After receiving a
...
- [**ListSnapshots:**](./Methods.mdx#listsnapshots) Used by nodes to discover available
  snapshots on peers.

- [**OfferSnapshot:**](./Methods.mdx#offersnapshot) When a node receives a snapshot from a
  peer, CometBFT uses this method to offer the snapshot to the Application.

- [**LoadSnapshotChunk:**](./Methods.mdx#loadsnapshotchunk) Used by CometBFT to retrieve
  snapshot chunks from the Application to send to peers.

- [**ApplySnapshotChunk:**](./Methods.mdx#applysnapshotchunk) Used by CometBFT to hand
  snapshot chunks to the Application.

### Other methods

Additionally, there is a [**Flush**](./Methods.mdx#flush) method that is called on every connection,
and an [**Echo**](./Methods.mdx#echo) method that is used for debugging.

More details on managing state across connections can be found in the section on
[Managing Application State](./Requirements-for-the-Application.mdx#managing-the-application-state-and-related-topics).
