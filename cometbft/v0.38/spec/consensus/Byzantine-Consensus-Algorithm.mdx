...
- A vote _at_ `(H,R)` is a vote signed with the bytes for `H` and `R`
  included in its [sign-bytes](/cometbft/v0.38/spec/core/Data_structures#vote).
...
- **Lemma**: When a fork is detected by the existence of two
  conflicting [commits](/cometbft/v0.38/spec/core/Data_structures#commit), the
...
[commit](https://github.com/cometbft/cometbft/blob/v0.38.x/docs/core/validators), any 1/3+ coalition of
