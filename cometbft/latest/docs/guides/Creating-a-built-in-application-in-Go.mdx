...
If that is the way you wish to proceed, use the [Creating an application in Go](/cometbft/latest/docs/guides/Creating-an-application-in-Go) guide instead of this one.
...
**Note:** `FinalizeBlock` only prepares the update to be made and does not change the state of the application. The state change is actually committed in a later stage, i.e., in the `commit` phase.
