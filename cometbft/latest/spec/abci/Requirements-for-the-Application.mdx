...
Since, as stated in the [Methods](/cometbft/latest/spec/abci/Methods#extendvote) section, `ResponseExtendVote`
...
[default in-process ABCI client](https://github.com/cometbft/cometbft/blob/v0.40.x/abci/client/local_client.go#L13)
and the
[default Go ABCI server](https://github.com/cometbft/cometbft/blob/v0.40.x/abci/server/socket_server.go#L20)
...
[`ExecTxResult`](/cometbft/latest/spec/abci/Methods#exectxresult) in
[`ResponseFinalizeBlock`](/cometbft/latest/spec/abci/Methods#finalizeblock). The list of transaction
results MUST respect the same order as the list of transactions delivered via
[`RequestFinalizeBlock`](/cometbft/latest/spec/abci/Methods#finalizeblock).
This section discusses the fields inside this structure, along with the fields in
[`ResponseCheckTx`](/cometbft/latest/spec/abci/Methods#checktx),
...
[`InitChain`](/cometbft/latest/spec/abci/Methods#initchain), and may update it during
[`FinalizeBlock`](/cometbft/latest/spec/abci/Methods#finalizeblock). In both cases, a structure of type
[`ValidatorUpdate`](/cometbft/latest/spec/abci/Methods#validatorupdate) is returned.
...
The public key currently supports the following types:
...
Structure `ValidatorUpdate` also contains an `int64` field denoting the validator's new power.
...
at block `H+2` (see Section [Methods](/cometbft/latest/spec/abci/Methods)).
...
[`InitChain`](/cometbft/latest/spec/abci/Methods#initchain), and updated in
[`FinalizeBlock`](/cometbft/latest/spec/abci/Methods#finalizeblock).
...
2. [AuthorityParams.Authority](#authorityparamsauthority)
3. [BlockParams.MaxBytes](#blockparamsmaxbytes)
4. [BlockParams.MaxGas](#blockparamsmaxgas)
5. [EvidenceParams.MaxAgeDuration](#evidenceparamsmaxageduration)
6. [EvidenceParams.MaxAgeNumBlocks](#evidenceparamsmaxagenumblocks)
7. [EvidenceParams.MaxBytes](#evidenceparamsmaxbytes)
...
An opaque, application-defined authority string. CometBFT does not interpret it and
only enforces a maximum length. The default is the empty string.
...
[timeout parameters](/cometbft/latest/docs/core/configuration#consensus-timeouts-explained)
...
> parameter accepts as valid blocks with size up to roughly 53 MiB: a 21 MiB
> budget for block data, plus a worst-case commit reserve sized for the maximum
> validator set at the maximum signature size. Enabling a large-signature key
> type such as `ml_dsa_65` is what makes that reserve large.
...
[`InitChain`](/cometbft/latest/spec/abci/Methods#initchain),
...
[methods](/cometbft/latest/spec/abci/Methods) section.
...
to switch are met [RFC-100](https://github.com/cometbft/cometbft/blob/v0.40.x/docs/references/rfc/rfc-100-abci-vote-extension-propag.md#base-implementation-persist-and-propagate-extended-commit-history).
...
[section](https://github.com/cometbft/cometbft/blob/v0.40.x/docs/references/rfc/rfc-100-abci-vote-extension-propag.md#upgrade-path) in RFC-100.

There is a newly introduced [**consensus parameter**](/cometbft/latest/spec/abci/Requirements-for-the-Application#consensus-parameters): `VoteExtensionsEnableHeight`.
...
Corollary, [CometBFT will decide](/cometbft/latest/spec/abci/CometBFTs-expected-behavior#handling-upgrades-to-abci-2-0) which data to store, and require for successful operations, based on the current height
of the chain.
