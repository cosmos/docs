...
| Data       | [Data](#data)                  | Data contains a list of transactions. The contents of the transaction is unknown to CometBFT.                                                                                      | This field can be empty or populated, but no validation is performed. Applications can perform validation on individual transactions prior to block creation using [checkTx](https://github.com/cometbft/cometbft/blob/v0.40.x/spec/abci/abci%2B%2B_methods.md#checktx).
...
| Time              | [Time](#time)             | The timestamp is equal to the weighted median of validators present in the last commit. Read more on time in the [BFT-time section](/cometbft/latest/spec/consensus/BFT-Time). Note: the timestamp of a vote must be greater by at least one millisecond than that of the block being voted on.                                                                                                       | Time must be >= previous header timestamp + consensus parameters TimeIotaMs.  The timestamp of the first block must be equal to the genesis time (since there's no votes to compute the median). |
...
| App   | uint64 | App version is decided on by the application. Read [here](https://github.com/cometbft/cometbft/blob/v0.40.x/spec/abci/abci++_app_requirements.md) | `block.Version.App == state.Version.Consensus.App`                                                              |
...
See [MerkleRoot](/cometbft/latest/spec/core/encoding#merkleroot) for details.
...
(see [ADR-25](https://github.com/cometbft/cometbft/blob/v0.40.x/docs/references/architecture/tendermint-core/adr-025-commit.md)).
...
See the [signature spec](/cometbft/latest/spec/core/encoding#key-types) for more.
...
More information on how evidence works in CometBFT can be found [here](/cometbft/latest/spec/consensus/Evidence)
...
and Amnesia. These attacks are exhaustive. You can find a more detailed overview of this [here](/cometbft/latest/spec/light-client/Accountability#the-misbehavior-of-faulty-validators)
...
LightBlock is the core data structure of the [light client](/cometbft/latest/spec/light-client/Light-Client-Specification). It combines two data structures needed for verification ([signedHeader](#signedheader) & [validatorSet](#validatorset)).

| Name         | Type                          | Description                                                                                                                            | Validation                                                                          |
|--------------|-------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------|
| SignedHeader | [SignedHeader](#signedheader) | The header and commit, these are used for verification purposes. To find out more visit [light client docs](/cometbft/latest/spec/light-client/Light-Client-Specification) | Must not be nil and adhere to the validation rules of [signedHeader](#signedheader) |
