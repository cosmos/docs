...
- To _prevote_ or _precommit_ something means to broadcast a prevote
  or precommit [vote](https://github.com/cometbft/cometbft/blob/v0.40.x/types/vote.go#L64-L75)
  for something.
- A vote _at_ `(H,R)` is a vote signed with the bytes for `H` and `R`
  included in its [sign-bytes](/cometbft/latest/spec/core/Data_structures#vote).
...
- Nodes gossip to nodes lagging in blockchain height with block
  [commits](https://github.com/cometbft/cometbft/blob/v0.40.x/types/block.go#L852-L866)
  for older blocks.
...
voting power (see
[implementation](https://github.com/cometbft/cometbft/blob/v0.40.x/types/validator_set.go#L56-L65)).
...
- **Lemma**: When a fork is detected by the existence of two
  conflicting [commits](/cometbft/latest/spec/core/Data_structures#commit), the
...
[commit](https://github.com/cometbft/cometbft/blob/v0.40.x/docs/core/validators.md), any 1/3+ coalition of
