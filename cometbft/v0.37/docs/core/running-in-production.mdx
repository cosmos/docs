...
result events. See [indexing transactions](../app-dev/indexing-transactions) for
details.
...
Validators are supposed to setup [Sentry Node Architecture](./validators)
...
[Debugging](../tools/debugging) for the exact format.
...
information into an archive. See [Debugging](../tools/debugging) for more
information.
