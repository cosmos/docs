...
Then, `ssh` into each machine and install CometBFT following the [instructions](./install).
...
Persistent peers can also be specified in the `config.toml`. See [here](../core/configuration) for more information about configuration options.
