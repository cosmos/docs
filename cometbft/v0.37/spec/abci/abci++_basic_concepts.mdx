...
[CometBFT's expected behavior](./abci++_comet_expected_behavior) for details on the possible
call sequences of these methods.

- [**InitChain:**](./abci++_methods#initchain) This method initializes the blockchain.
  CometBFT calls it once upon genesis.

- [**PrepareProposal:**](./abci++_methods#prepareproposal) It allows the block
...
- [**ProcessProposal:**](./abci++_methods#processproposal) It allows a validator to
...
- [**BeginBlock:**](./abci++_methods#beginblock) Is called exactly once after a block has been decided
  and executes once before all `DeliverTx` method calls.

- [**DeliverTx**](./abci++_methods#delivertx) Upon completion of `BeginBlock`,
...
- [**EndBlock**](./abci++_methods#endblock) It is executed once all transactions have been processed via
...
- [**Commit:**](./abci++_methods#commit) Instructs the Application to persist its
...
- [**CheckTx:**](./abci++_methods#checktx) This method allows the Application to validate
...
- [**Info:**](./abci++_methods#info) Used to sync CometBFT with the Application during a
  handshake that happens upon recovery, or on startup when state-sync is used.

- [**Query:**](./abci++_methods#query) This method can be used to query the Application for
...
[snapshot data type](./abci++_methods#snapshot) for more details). After receiving a
...
[light client verification](../light-client/verification/README)).

In summary:

- [**ListSnapshots:**](./abci++_methods#listsnapshots) Used by nodes to discover available
  snapshots on peers.

- [**OfferSnapshot:**](./abci++_methods#offersnapshot) When a node receives a snapshot from a
  peer, CometBFT uses this method to offer the snapshot to the Application.

- [**LoadSnapshotChunk:**](./abci++_methods#loadsnapshotchunk) Used by CometBFT to retrieve
  snapshot chunks from the Application to send to peers.

- [**ApplySnapshotChunk:**](./abci++_methods#applysnapshotchunk) Used by CometBFT to hand
  snapshot chunks to the Application.

### Other methods

Additionally, there is a [**Flush**](./abci++_methods#flush) method that is called on every connection,
and an [**Echo**](./abci++_methods#echo) method that is used for debugging.

More details on managing state across connections can be found in the section on
[Managing Application State](./abci%2B%2B_app_requirements#managing-the-application-state-and-related-topics).
