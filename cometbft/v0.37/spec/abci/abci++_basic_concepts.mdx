...
[CometBFT's expected behavior](/cometbft/v0.37/spec/abci/abci++_comet_expected_behavior) for details on the possible
call sequences of these methods.

- [**InitChain:**](/cometbft/v0.37/spec/abci/abci++_methods#initchain) This method initializes the blockchain.
  CometBFT calls it once upon genesis.

- [**PrepareProposal:**](/cometbft/v0.37/spec/abci/abci++_methods#prepareproposal) It allows the block
...
- [**ProcessProposal:**](/cometbft/v0.37/spec/abci/abci++_methods#processproposal) It allows a validator to
...
- [**BeginBlock:**](/cometbft/v0.37/spec/abci/abci++_methods#beginblock) Is called exactly once after a block has been decided
  and executes once before all `DeliverTx` method calls.

- [**DeliverTx**](/cometbft/v0.37/spec/abci/abci++_methods#delivertx) Upon completion of `BeginBlock`,
...
- [**EndBlock**](/cometbft/v0.37/spec/abci/abci++_methods#endblock) It is executed once all transactions have been processed via
...
- [**Commit:**](/cometbft/v0.37/spec/abci/abci++_methods#commit) Instructs the Application to persist its
...
- [**CheckTx:**](/cometbft/v0.37/spec/abci/abci++_methods#checktx) This method allows the Application to validate
...
- [**Info:**](/cometbft/v0.37/spec/abci/abci++_methods#info) Used to sync CometBFT with the Application during a
  handshake that happens upon recovery, or on startup when state-sync is used.

- [**Query:**](/cometbft/v0.37/spec/abci/abci++_methods#query) This method can be used to query the Application for
...
[state sync documentation](https://github.com/cometbft/cometbft/blob/v0.37.x/spec/p2p/legacy-docs/messages/state-sync.md).
...
[snapshot data type](/cometbft/v0.37/spec/abci/abci++_methods#snapshot) for more details). After receiving a
...
[light client verification](/cometbft/v0.37/spec/light-client/verification/README)).

In summary:

- [**ListSnapshots:**](/cometbft/v0.37/spec/abci/abci++_methods#listsnapshots) Used by nodes to discover available
  snapshots on peers.

- [**OfferSnapshot:**](/cometbft/v0.37/spec/abci/abci++_methods#offersnapshot) When a node receives a snapshot from a
  peer, CometBFT uses this method to offer the snapshot to the Application.

- [**LoadSnapshotChunk:**](/cometbft/v0.37/spec/abci/abci++_methods#loadsnapshotchunk) Used by CometBFT to retrieve
  snapshot chunks from the Application to send to peers.

- [**ApplySnapshotChunk:**](/cometbft/v0.37/spec/abci/abci++_methods#applysnapshotchunk) Used by CometBFT to hand
  snapshot chunks to the Application.

### Other methods

Additionally, there is a [**Flush**](/cometbft/v0.37/spec/abci/abci++_methods#flush) method that is called on every connection,
and an [**Echo**](/cometbft/v0.37/spec/abci/abci++_methods#echo) method that is used for debugging.

More details on managing state across connections can be found in the section on
[Managing Application State](/cometbft/v0.37/spec/abci/abci++_app_requirements#managing-the-application-state-and-related-topics).
...
information can be found in either [data structures](/cometbft/v0.37/spec/core/data_structures)
or [accountability](/cometbft/v0.37/spec/light-client/accountability/README).
