...
Further information on [`Validator`'s](./data_structures#validator),
[`ValidatorSet`'s](./data_structures#validatorset) and
[`ConsensusParams`'s](./data_structures#consensusparams) can
be found in [data structures](./data_structures)
...
[`Consensus`](./data_structures#version) contains the protocol version for the blockchain and the
application.
