...
```go "EthAccountI Interface" expandable
...
**EIP-7702 Support**: Cosmos EVM supports [EIP-7702 code delegation](/docs/evm/v0.5.0/documentation/evm-compatibility/eip-7702), allowing EOAs to temporarily delegate code execution to smart contracts through the `SetCodeHash` functionality.

For more information on Ethereum accounts head over to the [x/vm module](/docs/evm/v0.5.0/documentation/cosmos-sdk/modules/vm).
...
<Info>
The Bech32 format is the default for Cosmos-SDK queries and transactions, while the hex format is used for Ethereum compatibility. Both formats are supported interchangeably in IBC transfers through the address codec.
</Info>
...
<Info>
For more information on address format usage in IBC transfers, see [IBC - Address Formats](/docs/evm/v0.5.0/documentation/concepts/ibc#address-formats-for-ibc-transfers).
</Info>
...
<Tabs>
  ...
  <Tab title="JSON-RPC">
    ```bash "JSON-RPC Account Queries" expandable
...
<Note>
For JSON-RPC methods, see [`eth_accounts`](/docs/evm/v0.5.0/api-reference/ethereum-json-rpc/methods#eth-accounts) and [`personal_listAccounts`](/docs/evm/v0.5.0/api-reference/ethereum-json-rpc/methods#personal-listAccounts) documentation.
</Note>
