...
<Info>
Smart contracts can initiate IBC transfers using the [ICS20 Precompile](/docs/evm/v0.5.0/documentation/smart-contracts/precompiles/ics20), which provides the `transfer` function with memo field support for callbacks.
</Info>

<Info>
**Address Format Support**: IBC transfer supports multiple address formats through the AddressCodec interface. Both hex addresses (e.g., `0x...`) and bech32 addresses (e.g., `cosmos1...`) are supported for sender and receiver addresses. The underlying chain's address codec handles validation and conversion between formats.

For more details, see [Custom Address Codec](/docs/ibc/v10.1.x/ibc/apps/address-codec) in the IBC documentation.
</Info>
...
- [IBC Overview](/docs/evm/v0.5.0/documentation/concepts/ibc) - IBC concepts and fundamentals
- [ICS20 Precompile](/docs/evm/v0.5.0/documentation/smart-contracts/precompiles/ics20) - Cross-chain token transfers
- [Callbacks Interface](/docs/evm/v0.5.0/documentation/smart-contracts/precompiles/callbacks) - Smart contract callback interface
