...
Add the EVM mempool to your chain for nonce gap handling and transaction prioritization:

```go expandable
// In app/app.go, add to your App struct
type App struct {
    *baseapp.BaseApp
    // ... other keepers
...
// In NewApp constructor (after setting antehandler)
if evmtypes.GetChainConfig() != nil {
    ...
    evmMempool := evmmempool.NewExperimentalEVMMempool(
        app.CreateQueryContext, logger, app.EVMKeeper,
        app.FeeMarketKeeper, app.txConfig, app.clientCtx, mempoolConfig,
    )
    app.EVMMempool = evmMempool
    app.SetMempool(evmMempool)

    // Set custom CheckTx handler for nonce gaps
...
Customize mempool behavior with advanced configuration:

```go expandable
// Custom EVM TxPool configuration
...
// Custom Cosmos mempool priority
...
cosmosCfg.TxPriority = sdkmempool.TxPriority[math.Int]{
    GetTxPriority: func(goCtx context.Context, tx sdk.Tx) math.Int {
        ...
        fee := feeTx.GetFee()
        found, coin := fee.Find("stake") // your bond denom
        if !found {
            return math.ZeroInt()
        }
        return coin.Amount.Quo(math.NewIntFromUint64(feeTx.GetGas()))
    },
...
mempoolConfig := &evmmempool.EVMMempoolConfig{
    ...
    LegacyPoolConfig: &legacyCfg,
    CosmosPoolConfig: &cosmosCfg,
}
```

See the [full mempool documentation](/docs/evm/v0.5.0/documentation/getting-started/build-a-chain/additional-configuration/mempool-integration) for details.
</Tab>
</Tabs>
...
<Tabs>
<Tab title="Genesis Deployment">
Deploy standard contracts (Create2, Multicall3, Permit2, Safe) at chain initialization:

```go expandable
// In your genesis configuration (e.g., evmd/genesis.go)
func NewEVMGenesisState() *evmtypes.GenesisState {
    evmGenState := evmtypes.DefaultGenesisState()
    evmGenState.Params.ActiveStaticPrecompiles = evmtypes.AvailableStaticPrecompiles
    evmGenState.Preinstalls = evmtypes.DefaultPreinstalls  // Includes all standard contracts
    return evmGenState
}
```

Default contracts included:
- **Create2** (`0x4e59b44847b379578588920ca78fbf26c0b4956c`) - Deterministic deployment
- **Multicall3** (`0xcA11bde05977b3631167028862bE2a173976CA11`) - Batched calls
- **Permit2** (`0x000000000022D473030F116dDEE9F6B43aC78BA3`) - Signature-based approvals
- **Safe Factory** (`0x914d7Fec6aaC8cd542e72Bca78B30650d45643d7`) - Multisig wallets
...
<Tab title="Custom Contracts">
Add your own predeployed contracts:
...
- Valid 40-character hex address (with 0x prefix)
- Non-empty EVM bytecode
- No conflicts with existing contracts or precompiles (0x1-0x9FF)
</Tab>

<Tab title="Governance Deployment">
Deploy contracts on running chains via governance:
...
```json expandable
{
  ...
  "summary": "Deploy Multicall3 for batched calls"
}
```

```bash
# Submit and vote
evmd tx gov submit-proposal proposal.json --from mykey
...
<Warning>
**Known Issue - Safe Factory**: The Safe Singleton Factory bytecode in `DefaultPreinstalls` is currently incorrect (identical to Create2). Before production deployment, get the correct bytecode from the [official Safe repository](https://github.com/safe-global/safe-singleton-factory). See the [predeployed contracts documentation](/docs/evm/v0.5.0/documentation/getting-started/build-a-chain/additional-configuration/predeployed-contracts) for details.
</Warning>
