...
<Note>
  This is part of cosmos/evm, not standard Cosmos SDK. For conceptual understanding of EIP-1559, see [EIP-1559 Fee Market](/docs/evm/next/documentation/concepts/eip-1559-feemarket).
</Note>
...
```json Ethereum-Compatible expandable
{
  "no_base_fee": false,
...
```json High-Volume-Stable expandable
{
  "no_base_fee": false,
...
```json Aggressive-Anti-Spam expandable
{
  "no_base_fee": false,
...
```json Governance Parameter Update expandable
...
```javascript Wallet-Integration expandable
...
<ResponseField name="eth_feeHistory" type="object">
  ...
  ```json eth_feeHistory Response expandable
  / Request
  {
    "jsonrpc":"2.0",
...
```proto Query-Parameters expandable
...
```http Query-Parameters expandable
...
```go BeginBlock Implementation expandable
...
```go EndBlock Implementation expandable
