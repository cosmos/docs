...
**File Location**: [`server/config/config.go:65`](https://github.com/cosmos/evm/blob/main/server/config/config.go#L64-L65)
...
<Tabs>
<Tab title="Option 1: Set in Source (Before Init)">
Edit `server/config/config.go` and change the `DefaultEVMChainID` value:

```go expandable
// DefaultEVMChainID is the default EVM Chain ID if one is not provided
DefaultEVMChainID = 123456  // Change to your unique chain ID
```
...
**Then initialize (your custom chain ID will be written to app.toml):**
```bash expandable
...
<Tab title="Option 2: Edit After Init">
Alternatively, just edit `app.toml` directly after running init:
...
# Then edit app.toml
# Change evm-chain-id = 262144 to your desired chain ID
```

**Edit `~/.yourchain/config/app.toml`:**
```toml expandable
[evm]
evm-chain-id = 123456  # Your unique chain ID
```

**This approach is simpler if you're only changing the chain ID without other source code modifications.**
</Tab>
