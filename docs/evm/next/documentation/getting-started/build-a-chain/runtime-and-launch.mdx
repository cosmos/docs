...
# Allow unprotected transactions (ONLY FOR DEV)
...
1. **Check whitespace**: Use `jq -S -c` to normalize
2. **Re-download**: Get genesis file again
3. **Verify source**: Ensure downloading from official source
4. **Check corruption**: File may be corrupted in transfer

**Even one character difference = different genesis state**

All validators must have byte-for-byte identical files.
</Tab>
</Tabs>
...
<Tabs>
<Tab title="Node Won't Start">
**Error: `invalid chain-id on InitChain`**
...
**Error: `genesis.json file not found`**

**Solution:**
```bash
# Verify file exists
ls ~/.yourchain/config/genesis.json

# Re-download if missing
```
</Tab>

<Tab title="No Peer Connections">
**Check P2P configuration:**

```bash
# Verify persistent_peers in config.toml
grep 'persistent_peers' ~/.yourchain/config/config.toml
...
**Check firewall:**
```bash
# Port 26656 must be open for P2P
sudo ufw status
sudo ufw allow 26656
```
</Tab>

<Tab title="Genesis Hash Mismatch">
**If validators have different genesis files:**

1. **Stop all nodes**
2. **Coordinator distributes canonical genesis**
3. **All validators re-verify hash**
4. **Restart launch process**
...
<Tab title="Blocks Not Producing">
**Check validator participation:**

```bash
# Check number of online validators
curl localhost:26657/validators | jq '.result.validators | length'
...
**Need >=2/3 voting power online for consensus.**

**If insufficient:**
- Wait for more validators to come online
- Verify genesis file distributed
