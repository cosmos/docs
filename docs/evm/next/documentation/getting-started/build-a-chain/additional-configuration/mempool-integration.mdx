...
The `server` package provides helpers to read configuration from `app.toml` and genesis:
...
mempoolConfig := &evmmempool.EVMMempoolConfig{
    AnteHandler:   app.GetAnteHandler(),
    BlockGasLimit: 100_000_000, // or use server.GetBlockGasLimit
}
...
- [Using the Local Node](/docs/evm/next/documentation/getting-started/build-a-chain/using-local-node#evm-mempool-integration) - Quick setup example
