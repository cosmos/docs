...
<Note>
  Big thanks to Reece & the [Spawn](https://github.com/rollchains/spawn) team
...
Contact [Cosmos Labs](https://cosmos.network/interest-form) for production chain upgrade guidance.
...
<Note>
  Throughout this guide, `evmd` refers to your chain's binary (e.g., `gaiad`,
  `dydxd`, etc.).
</Note>
...
<Warning>
  These version numbers may change as development continues. Check
  [github.com/cosmos/evm](https://github.com/cosmos/evm) for the latest
  releases.
</Warning>
...
<Note>
  Ensure your EVM chain ID is not already in use by checking
...
<Warning>
  The mismatch between EVM's 18-decimal standard and Cosmos SDK's 6-decimal
  standard is critical. The default behavior (flooring) discards any value below
  10^-6, causing asset loss and breaking DeFi applications.
</Warning>
