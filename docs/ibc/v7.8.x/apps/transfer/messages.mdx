...
* `Token` is invalid (denom is invalid or amount is negative)
  ...
  * `Token.Denom` is not a valid IBC denomination as per [ADR 001 - Coin Source Tracing](/docs/ibc/v7.8.x/architecture/adr-001-coin-source-tracing).
* `Sender` is empty.
...
For example, the following memo field is used by the [callbacks middleware](/docs/ibc/v7.8.x//04-middleware/02-callbacks/01-overview) to attach a source callback to a transfer packet:
