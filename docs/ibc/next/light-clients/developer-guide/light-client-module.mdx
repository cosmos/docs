...
`UpdateState` updates and stores as necessary any associated information for an IBC client, such as the `ClientState` and corresponding `ConsensusState`. See section [`UpdateState`](/docs/ibc/next/light-clients/developer-guide/updates-and-misbehaviour.mdx#updatestate) for more information.

## `UpdateStateOnMisbehaviour` method

`UpdateStateOnMisbehaviour` should perform appropriate state changes on a client state given that misbehaviour has been detected and verified. See section [`UpdateStateOnMisbehaviour`](/docs/ibc/next/light-clients/developer-guide/updates-and-misbehaviour.mdx#updatestateonmisbehaviour) for more information.
...
see the [Existence and non-existence proofs section](/docs/ibc/next/light-clients/developer-guide/proofs.mdx).
