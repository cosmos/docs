...
`UpdateState` updates and stores as necessary any associated information for an IBC client, such as the `ClientState` and corresponding `ConsensusState`. See section [`UpdateState`](updates-and-misbehaviour#updatestate) for more information.

## `UpdateStateOnMisbehaviour` method

`UpdateStateOnMisbehaviour` should perform appropriate state changes on a client state given that misbehaviour has been detected and verified. See section [`UpdateStateOnMisbehaviour`](updates-and-misbehaviour#updatestateonmisbehaviour) for more information.
...
see the [Existence and non-existence proofs section](proofs).
...
if the ClientMessage fails to verify. See section [`VerifyClientMessage`](updates-and-misbehaviour#verifyclientmessage) for more information.

## `CheckForMisbehaviour` method

Checks for evidence of a misbehaviour in `Header` or `Misbehaviour` type. It assumes the `ClientMessage`
has already been verified. See section [`CheckForMisbehaviour`](updates-and-misbehaviour#checkformisbehaviour) for more information.
