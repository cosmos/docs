...
To supplement this, a [sentinel `ConnectionEnd` is stored in core IBC](connection) state with the connection
...
[State verification](state-verification) for channel state in handshakes or processing packets is reduced in
