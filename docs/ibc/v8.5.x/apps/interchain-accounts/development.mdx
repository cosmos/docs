...
The underlying application may continue to be used as a short term solution for ADR 008 and the [legacy API](/docs/ibc/v8.5.x/apps/interchain-accounts/legacy/auth-modules) should continue to be utilized in such situations.
...
In the first case, the smart contract should use the [`MsgServer`](/docs/ibc/v8.5.x/apps/interchain-accounts/messages.mdx).

In the second case, the underlying application should use the [legacy API](/docs/ibc/v8.5.x/apps/interchain-accounts/apps/interchain-accounts/legacy/keeper-api.mdx).
