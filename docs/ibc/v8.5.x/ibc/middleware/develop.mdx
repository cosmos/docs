...
As will be mentioned in the [integration section](/docs/ibc/v8.5.x/ibc/middleware/integration), this struct should be different than the struct that implements `AppModule` in case the middleware maintains its own internal state and processes separate SDK messages.
