...
* [Keepers](/docs/sdk/vnext/build/building-modules/keeper)
...
The `InvariantRegistry` is therefore typically instantiated by instantiating the `keeper` of the `crisis` module in the [application's constructor function](/docs/sdk/vnext/learn/beginner/app-anatomy#constructor-function).

`Invariant`s can be checked manually via [`message`s](/docs/sdk/vnext/build/building-modules/messages-and-queries), but most often they are checked automatically at the end of each block. Here is an example from the `crisis` module:
