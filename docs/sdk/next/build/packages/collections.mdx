...
Under the hood the `collections.Map` will convert the key and value to bytes using the [key and value codec](/docs/sdk/vnext/build/packages/README#key-and-value-codecs).
It will prepend to our bytes key the [prefix](/docs/sdk/vnext/build/packages/README#prefix) and store it in the KVStore of the module.
