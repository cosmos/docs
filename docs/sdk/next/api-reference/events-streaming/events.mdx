...
They are triggered from the module's Protobuf [`Msg` service](/docs/sdk/next//build/building-modules/msg-services)
...
| `message.action='/cosmos.bank.v1beta1.Msg/Send'` | Query all transactions containing a x/bank `Send` [Service `Msg`](/docs/sdk/next//build/building-modules/msg-services). Note the `'`s around the value. |
...
See the [`Msg` services](/docs/sdk/next//build/building-modules/msg-services) concept doc for a more detailed
