...
[`depinject`](/docs/sdk/next/packages/depinject) is used to wire any module in `app.go`.
...
The App Wiring is done in `app_config.go` / `app.yaml` and `app_di.go` and is explained in detail in the [overview of `app_di.go`](/docs/sdk/next/building-apps/app-go-di).
