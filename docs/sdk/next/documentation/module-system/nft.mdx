...
`x/nft` is an implementation of a Cosmos SDK module, per [ADR 43](/docs/sdk/next/documentation/legacy/adr-comprehensive#adr-043-nft-module), that allows you to create nft classification, create nft, transfer nft, update nft, and support various queries by integrating the module. It is fully compatible with the ERC721 specification.
...
`x/nft` module defines a struct `Class` to describe the common characteristics of a class of nft, under this class, you can create a variety of nft, which is equivalent to an erc721 contract for Ethereum. The design is defined in the [ADR 043](docs/sdk/next/documentation/legacy/adr-comprehensive).
...
<Warning>
  The validation of `ClassID` and `NftID` is left to the app developer.\ The SDK
  does not provide any validation for these fields.
</Warning>
