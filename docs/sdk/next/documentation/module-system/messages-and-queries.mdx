...
Read more about the signer field [here](/docs/sdk/next/protobuf-annotations).
...
* `MsgServer` interface defines the server API for the `Msg` service and its implementation is described as part of the [`Msg` services](/docs/sdk/next/msg-services) documentation.
* Structures are generated for all RPC request and response types.

A `RegisterMsgServer` method is also generated and should be used to register the module's `MsgServer` implementation in `RegisterServices` method from the [`AppModule` interface](/docs/sdk/next/module-manager#appmodule).
...
A `RegisterQueryServer` method is also generated and should be used to register the module's query server in the `RegisterServices` method from the [`AppModule` interface](/docs/sdk/next/module-manager#appmodule).
...
* `queryType` is used by the module's [`querier`](/docs/sdk/next/query-services#legacy-queriers) to map the `query` to the appropriate `querier function` within the module.
...
The `path` for each `query` must be defined by the module developer in the module's [command-line interface file](/docs/sdk/next/module-interfaces#query-commands). Overall, there are 3 mains components module developers need to implement in order to make the subset of the state defined by their module queryable:

* A [`querier`](/docs/sdk/next/query-services#legacy-queriers), to process the `query` once it has been [routed to the module](/docs/sdk/next/documentation/application-framework/baseapp#query-routing).
* [Query commands](/docs/sdk/next/module-interfaces#query-commands) in the module's CLI file, where the `path` for each `query` is specified.
