...
* `AddQueryCommands(rootQueryCmd *cobra.Command)`: Adds modules' query commands (defined as `GetQueryCmd() *cobra.Command`) to the application's [`rootQueryCommand`](/docs/sdk/next/api-reference/client-tools/cli#query-commands). This function is usually called function from the `main.go` function of the [application's command-line interface](/docs/sdk/next/api-reference/client-tools/cli).
...
* `SetOrderInitGenesis(moduleNames ...string)`: Sets the order in which the [`InitGenesis`](/docs/sdk/next/genesis#initgenesis) function of each module will be called when the application is first started. This function is generally called from the application's main [constructor function](/docs/sdk/next/documentation/application-framework/app-anatomy#constructor-function).
