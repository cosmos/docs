...
Location: off-chain as json object stored on IPFS (mirrors [gov proposal](/docs/sdk/next/documentation/module-system/gov#metadata))
...
<Note>
  The `authors` field is an array of strings, this is to allow for multiple
  authors to be listed in the metadata. In v0.46, the `authors` field is a
  comma-separated string. Frontends are encouraged to support both formats for
  backwards compatibility.
</Note>

### Vote

Location: on-chain as json within 255 character limit (mirrors [gov vote](/docs/sdk/next/documentation/module-system/gov#metadata))
