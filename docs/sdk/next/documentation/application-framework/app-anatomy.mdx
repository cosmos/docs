...
* Instantiate a new [`codec`](/docs/sdk/next/advanced/encoding) and initialize the `codec` of each of the application's modules using the [basic manager](/docs/sdk/next/documentation/module-system/module-manager#basicmanager).
...
gRPC `Query` services allow users to query the state using [gRPC](https://grpc.io). They are enabled by default, and can be configured under the `grpc.enable` and `grpc.address` fields inside [`app.toml`](/docs/sdk/next/documentation/operations/run-node#configuring-the-node-using-apptoml-and-configtoml).
