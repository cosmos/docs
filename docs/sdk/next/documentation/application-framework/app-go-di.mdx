...
The Cosmos SDK makes wiring of an `app.go` much easier thanks to [runtime](/docs/sdk/next/runtime) and app wiring.
Learn more about the rationale of App Wiring in [ADR-057](/docs/sdk/next/architecture/adr-057-app-wiring).
...
* [What is `runtime`?](/docs/sdk/next/runtime)
* [Depinject documentation](/docs/sdk/next/packages/depinject)
* [Modules depinject-ready](/docs/sdk/next/building-modules/depinject)
* [ADR 057: App Wiring](/docs/sdk/next/architecture/adr-057-app-wiring)
...
1. Create the `AppConfig` variable:
   ...
   Where the `appConfig` combines the [runtime](/docs/sdk/next/runtime) configuration and the (extra) modules configuration.
...
In short `depinject` and the [`runtime` package](/docs/sdk/next/runtime) abstract the wiring of the app, and the `AppBuilder` is the place where the app is constructed. [`runtime`](/docs/sdk/next/runtime) takes care of registering the codecs, KV store, subspaces and instantiating `baseapp`.
