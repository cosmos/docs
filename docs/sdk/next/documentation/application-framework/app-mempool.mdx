...
- [BaseApp](docs/sdk/next/documentation/application-framework/baseapp)
- [ABCI](docs/sdk/next/documentation/consensus-block-production/introduction)
...
By default, the SDK uses the [No-op Mempool](#no-op-mempool), but it can be replaced by the application developer in [`app.go`](/docs/sdk/next/documentation/application-framework/app-go-di):
...
It is an integer value that sets the mempool in one of three modes, _bounded_, _unbounded_, or _disabled_.
