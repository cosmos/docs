...
Additionally, please keep in mind that performance can be degraded if vote extensions are too big ([Link](https://docs.cometbft.com/v0.38/qa/cometbft-qa-38#vote-extensions-testbed)), so we highly recommend a size validation in `VerifyVoteExtensions`.
...
details on propagation, see the [ABCI++ 2.0 ADR](docs/sdk/next/documentation/legacy/adr-comprehensive#vote-extension-propagation--verification).
...
```go expandable
app.SetPreBlocker(func(ctx sdk.Context, req *abci.FinalizeBlockRequest)
