...
*Typed Events* are Protobuf-defined [messages](/docs/sdk/next///architecture/adr-032-typed-events) used by the Cosmos SDK
...
They are triggered from the module's Protobuf [`Msg` service](/docs/sdk/next//build/building-modules/03-msg-services)
...
* [`BeginBlock`](/docs/sdk/next/00-baseapp#beginblock)
* [`EndBlock`](/docs/sdk/next/00-baseapp#endblock)
* [`CheckTx`](/docs/sdk/next/00-baseapp#checktx)
* [`Transaction Execution`](/docs/sdk/next/00-baseapp#transactionexecution)
...
| `message.action='/cosmos.bank.v1beta1.Msg/Send'` | Query all transactions containing a x/bank `Send` [Service `Msg`](/docs/sdk/next//build/building-modules/03-msg-services). Note the `'`s around the value. |
...
the [`Context`](/docs/sdk/next/02-context), where Event should be already registered, and emitted like this:
...
Where the `EventManager` is accessed via the [`Context`](/docs/sdk/next/02-context).

See the [`Msg` services](/docs/sdk/next//build/building-modules/03-msg-services) concept doc for a more detailed
...
where `ownerAddress` is an address following the [`AccAddress`](/docs/sdk/next/beginner/03-accounts#addresses) format.
