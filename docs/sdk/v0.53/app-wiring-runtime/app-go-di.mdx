...
1. Create the `AppConfig` variable:
    ...
    Where the `appConfig` combines the [runtime](/docs/sdk/v0.53/00-runtime.md) configuration and the (extra) modules configuration.
...
In short `depinject` and the [`runtime` package](/docs/sdk/v0.53/00-runtime.md) abstract the wiring of the app, and the `AppBuilder` is the place where the app is constructed. [`runtime`](/docs/sdk/v0.53/00-runtime.md) takes care of registering the codecs, KV store, subspaces and instantiating `baseapp`.
