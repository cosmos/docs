...
The rationale behind testing can be found in [ADR-59](/docs/sdk/v0.53/build/architecture/adr-059-test-scopes).
...
In order to only test `x/gov`, we mock the [expected keepers](/docs/sdk/v0.53/building-modules/keeper.html#type-definition) and instantiate the `Keeper` with the mocked dependencies. Note that we may need to configure the mocked dependencies to return the expected values:
...
Learn more about testing scope in [ADR-59](/docs/sdk/v0.53/architecture/adr-059-test-scopes.html).
