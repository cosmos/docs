...
[`depinject`](/docs/sdk/v0.53/documentation/module-system/depinject) is used to wire any module in `app.go`.
...
The App Wiring is done in `app_config.go` / `app.yaml` and `app_di.go` and is explained in detail in the [overview of `app_di.go`](/docs/sdk/v0.53/documentation/application-framework/app-go-di).
