...
When possible, the existing module's [`Keeper`](/docs/sdk/v0.53/06-keeper.md) should implement `MsgServer`, otherwise a `msgServer` struct that embeds the `Keeper` can be created, typically in `./keeper/msg_server.go`:
...
After the validation is successful, the `msgServer` method uses the [`keeper`](/docs/sdk/v0.53/06-keeper.md) functions to access the state and perform a state transition.
