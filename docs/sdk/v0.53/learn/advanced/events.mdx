...
They are triggered from the module's Protobuf [`Msg` service](/docs/sdk/v0.53//build/building-modules/03-msg-services)
...
* [`BeginBlock`](/docs/sdk/v0.53/00-baseapp#beginblock)
* [`EndBlock`](/docs/sdk/v0.53/00-baseapp#endblock)
* [`CheckTx`](/docs/sdk/v0.53/00-baseapp#checktx)
* [`Transaction Execution`](/docs/sdk/v0.53/00-baseapp#transactionexecution)
...
| `message.action='/cosmos.bank.v1beta1.Msg/Send'` | Query all transactions containing a x/bank `Send` [Service `Msg`](/docs/sdk/v0.53//build/building-modules/03-msg-services). Note the `'`s around the value. |
...
the [`Context`](/docs/sdk/v0.53/02-context), where Event should be already registered, and emitted like this:
...
Where the `EventManager` is accessed via the [`Context`](/docs/sdk/v0.53/02-context).

See the [`Msg` services](/docs/sdk/v0.53//build/building-modules/03-msg-services) concept doc for a more detailed
...
where `ownerAddress` is an address following the [`AccAddress`](/docs/sdk/v0.53/beginner/03-accounts#addresses) format.
