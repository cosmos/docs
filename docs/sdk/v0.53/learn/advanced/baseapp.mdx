...
* [Anatomy of a Cosmos SDK application](/docs/sdk/v0.53/beginner/00-app-anatomy)
* [Lifecycle of a Cosmos SDK transaction](/docs/sdk/v0.53/beginner/01-tx-lifecycle)
...
* [`CommitMultiStore`](/docs/sdk/v0.53/04-store#commitmultistore): This is the main store of the application,
...
* [`InitChainer`](/docs/sdk/v0.53/beginner/00-app-anatomy#initchainer), [`PreBlocker`](/docs/sdk/v0.53/beginner/00-app-anatomy#preblocker), [`BeginBlocker` and `EndBlocker`](/docs/sdk/v0.53/beginner/00-app-anatomy#beginblocker-and-endblocker): These are
...
* `voteInfos`: This parameter carries the list of validators whose precommit is missing, either
  ...
  carried by the [Context](/docs/sdk/v0.53/02-context) and can be used by the application for various things like
...
* `appVersion`: Version of the application. It is set in the
  [application's constructor function](/docs/sdk/v0.53/beginner/00-app-anatomy#constructor-function).
...
The application's `msgServiceRouter` is initialized with all the routes using the application's [module manager](/docs/sdk/v0.53//build/building-modules/01-module-manager#manager) (via the `RegisterServices` method), which itself is initialized with all the application's modules in the application's [constructor](/docs/sdk/v0.53/beginner/00-app-anatomy#constructor-function).

### gRPC Query Router

Similar to `sdk.Msg`s, [`queries`](/docs/sdk/v0.53//build/building-modules/02-messages-and-queries#queries) need to be routed to the appropriate module's [`Query` service](/docs/sdk/v0.53//build/building-modules/04-query-services). To do so, `BaseApp` holds a `grpcQueryRouter`, which maps modules' fully-qualified service methods (`string`, defined in their Protobuf `Query` gRPC) to their `QueryServer` implementation. The `grpcQueryRouter` is called during the initial stages of query processing, which can be either by directly sending a gRPC query to the gRPC endpoint, or via the [`Query` ABCI message](#query) on the CometBFT RPC endpoint.

Just like the `msgServiceRouter`, the `grpcQueryRouter` is initialized with all the query routes using the application's [module manager](/docs/sdk/v0.53//build/building-modules/01-module-manager) (via the `RegisterServices` method), which itself is initialized with all the application's modules in the application's [constructor](/docs/sdk/v0.53/beginner/00-app-anatomy#app-constructor).
...
In the Cosmos SDK, after [decoding transactions](/docs/sdk/v0.53/05-encoding), `CheckTx()` is implemented
...
3. Perform non-module related *stateful* checks on the [account](/docs/sdk/v0.53/beginner/03-accounts). This step is mainly about checking
   that the `sdk.Msg` signatures are valid, that enough fees are provided and that the sending account
   has enough funds to pay for said fees. Note that no precise [`gas`](/docs/sdk/v0.53/beginner/04-gas-fees) counting occurs here,
   as `sdk.Msg`s are not processed. Usually, the [`AnteHandler`](/docs/sdk/v0.53/beginner/04-gas-fees#antehandler) will check that the `gas` provided
...
Steps 2. and 3. are performed by the [`AnteHandler`](/docs/sdk/v0.53/beginner/04-gas-fees#antehandler) in the [`RunTx()`](#runtx-antehandler-and-runmsgs)
...
* `Events ([]cmn.KVPair)`: Key-Value tags for filtering and indexing transactions (eg. by account). See [`event`s](/docs/sdk/v0.53/08-events) for more.
...
This allows `RunTx` not to commit the changes made to the state during the execution of `anteHandler` if it ends up failing. It also prevents the module implementing the `anteHandler` from writing to state, which is an important part of the [object-capabilities](/docs/sdk/v0.53/10-ocap) of the Cosmos SDK.
...
* Be a primary line of defense against spam and second line of defense (the first one being the mempool) against transaction replay with fees deduction and [`sequence`](/docs/sdk/v0.53/01-transactions#transaction-generation) checking.
...
`BaseApp` holds an `anteHandler` as parameter that is initialized in the [application's constructor](/docs/sdk/v0.53/beginner/00-app-anatomy#application-constructor). The most widely used `anteHandler` is the [`auth` module](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/x/auth/ante/ante.go).

Click [here](/docs/sdk/v0.53/beginner/04-gas-fees#antehandler) for more on the `anteHandler`.
...
* The [block gas meter](/docs/sdk/v0.53/beginner/04-gas-fees#block-gas-meter), with infinite gas to process genesis transactions.

Finally, the `InitChain(req abci.RequestInitChain)` method of `BaseApp` calls the [`initChainer()`](/docs/sdk/v0.53/beginner/00-app-anatomy#initchainer) of the application in order to initialize the main state of the application from the `genesis file` and, if defined, call the [`InitGenesis`](/docs/sdk/v0.53//build/building-modules/08-genesis#initgenesis) function of each of the application's modules.
...
* Run the application's [`preBlocker()`](/docs/sdk/v0.53/beginner/00-app-anatomy#preblocker), which mainly runs the [`PreBlocker()`](/docs/sdk/v0.53//build/building-modules/17-preblock#preblock) method of each of the modules.
...
* Initialize [`finalizeBlockState`](#state-updates) with the latest header using the `req abci.RequestFinalizeBlock` passed as parameter via the `setState` function.
  ...
  This function also resets the [main gas meter](/docs/sdk/v0.53/beginner/04-gas-fees#main-gas-meter).

* Initialize the [block gas meter](/docs/sdk/v0.53/beginner/04-gas-fees#block-gas-meter) with the `maxGas` limit. The `gas` consumed within the block cannot go above `maxGas`. This parameter is defined in the application's consensus parameters.

* Run the application's [`beginBlocker()`](/docs/sdk/v0.53/beginner/00-app-anatomy#beginblocker-and-endblocker), which mainly runs the [`BeginBlocker()`](/docs/sdk/v0.53//build/building-modules/06-beginblock-endblock#beginblock) method of each of the modules.

* Set the [`VoteInfos`](https://github.com/cometbft/cometbft/blob/v0.37.x/spec/abci/abci++_methods.md#voteinfo) of the application, i.e. the list of validators whose *precommit* for the previous block was included by the proposer of the current block. This information is carried into the [`Context`](/docs/sdk/v0.53/02-context) so that it can be used during transaction execution and EndBlock.
...
2. For each `sdk.Msg` in the transaction, route to the appropriate module's Protobuf [`Msg` service](/docs/sdk/v0.53//build/building-modules/03-msg-services). Additional *stateful* checks are performed, and the branched multistore held in `finalizeBlockState`'s `context` is updated by the module's `keeper`. If the `Msg` service returns successfully, the branched multistore held in `context` is written to `finalizeBlockState` `CacheMultiStore`.
