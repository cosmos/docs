...
<Note>
**Synopsis**
This document describes the lifecycle of a transaction from creation to committed state changes. Transaction definition is described in a [different doc](/docs/sdk/v0.53/learn/advanced/transactions). The transaction is referred to as `Tx`.
</Note>
...
Additionally, there are several [flags](/docs/sdk/v0.53/learn/advanced/cli) users can use to indicate how much they are willing to pay in [fees](/docs/sdk/v0.53/gas-fees):
...
At any given moment, full-nodes typically have [multiple versions](/docs/sdk/v0.53/learn/advanced/baseapp#state-updates)
...
When `Tx` is received by the application from the underlying consensus engine (e.g. CometBFT ), it is still in its [encoded](/docs/sdk/v0.53/learn/advanced/encoding) `[]byte` form and needs to be unmarshaled in order to be processed. Then, the [`runTx`](/docs/sdk/v0.53/learn/advanced/baseapp#runtx-antehandler-runmsgs-posthandler) function is called to run in `runTxModeCheck` mode, meaning the function runs all checks but exits before executing messages and writing state changes.

### ValidateBasic (deprecated)

Messages ([`sdk.Msg`](/docs/sdk/v0.53/learn/advanced/transactions#messages)) are extracted from transactions (`Tx`). The `ValidateBasic` method of the `sdk.Msg` interface implemented by the module developer is run for each transaction.
To discard obviously invalid messages, the `BaseApp` type calls the `ValidateBasic` method very early in the processing of the message in the [`CheckTx`](/docs/sdk/v0.53/learn/advanced/baseapp#checktx) and [`DeliverTx`](/docs/sdk/v0.53/learn/advanced/baseapp#delivertx) transactions.
...
The `FinalizeBlock` ABCI function defined in [`BaseApp`](/docs/sdk/v0.53/learn/advanced/baseapp) does the bulk of the
...
[`runTx`](/docs/sdk/v0.53/learn/advanced/baseapp#runtx) function in deliver mode instead of check mode.
...
* **`MsgServiceRouter`:** After `CheckTx` exits, `FinalizeBlock` continues to run
  [`runMsgs`](/docs/sdk/v0.53/learn/advanced/baseapp#runtx-antehandler-runmsgs-posthandler) to fully execute each `Msg` within the transaction.
  ...
  For `LegacyMsg` routing, the `Route` function is called via the [module manager](/docs/sdk/v0.53//build/building-modules/docs/sdk/v0.53/build/building-modules/module-manager) to retrieve the route name and find the legacy [`Handler`](/docs/sdk/v0.53//build/building-modules/msg-services#handler-type) within the module.
...
* **PostHandlers:** [`PostHandler`](/docs/sdk/v0.53/learn/advanced/baseapp#posthandler)s run after the execution of the message. If they fail, the state change of `runMsgs`, as well of `PostHandlers`, are both reverted.
...
a merkle proof for the state transitions. Applications use the [`Commit`](/docs/sdk/v0.53/learn/advanced/baseapp#commit)
ABCI method inherited from [Baseapp](/docs/sdk/v0.53/learn/advanced/baseapp); it syncs all the state transitions by
