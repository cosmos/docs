...
[ADR 019](/docs/sdk/v0.50/adr-019-protobuf-state-encoding), namely, we aim to design the
...
Since interface values are encoded with `google.protobuf.Any` in state (see [ADR 019](./docs/sdk/v0.50/build/architecture/adr-019-protobuf-state-encoding.mdx)),
...
2. Create a `SignDoc` and serialize it using [ADR 027](/docs/sdk/v0.50/adr-027-deterministic-protobuf-serialization).
...
3. For each required signer:
   ...
   * Create a `SignDoc` and serialize it using [ADR 027](/docs/sdk/v0.50/adr-027-deterministic-protobuf-serialization).
...
in the client can be interfaces, similar to how we described in [ADR 019](/docs/sdk/v0.50/adr-019-protobuf-state-encoding),
