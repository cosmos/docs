...
<Note>
**Synopsis**
A Protobuf Query service processes [`queries`](/docs/sdk/v0.50/build/building-modules/messages-and-queries#queries). Query services are specific to the module in which they are defined, and only process `queries` defined within said module. They are called from `BaseApp`'s [`Query` method](/docs/sdk/v0.50/learn/advanced/baseapp#query).
</Note>
