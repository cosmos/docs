...
When users want to interact with an application and make state changes (e.g. sending coins), they create transactions. Each of a transaction's `sdk.Msg` must be signed using the private key associated with the appropriate account(s), before the transaction is broadcasted to the network. A transaction must then be included in a block, validated, and approved by the network through the consensus process. To read more about the lifecycle of a transaction, click [here](/docs/sdk/v0.50/learn/beginner/tx-lifecycle).
...
If you wish to learn more, please refer to [ADR-050](https://github.com/cosmos/cosmos-sdk-docs/blob/main/docs/build/architecture/adr-050-sign-mode-textual.md).
...
To learn more about Protobuf `Msg` services and how to implement `MsgServer`, click [here](/docs/sdk/v0.50/documentation/module-system/msg-services).
