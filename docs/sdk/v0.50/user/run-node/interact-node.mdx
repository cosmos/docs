...
[CORS policies](https://developer.mozilla.org/en-US/docs/sdk/0.5/Web/HTTP/CORS) are not enabled by default to help with security. If you would like to use the rest-server in a public environment we recommend you provide a reverse proxy, this can be done with [nginx](https://www.nginx.com/). For testing and development purposes there is an `enabled-unsafe-cors` field inside [`app.toml`](/run-node/01-run-node.md#configuring-the-node-using-apptoml-and-configtoml).
