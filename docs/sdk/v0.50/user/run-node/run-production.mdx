...
Nodes should not have all ports open to the public, this is a simple way to get DDOS'd. Secondly it is recommended by [CometBFT](/docs/sdk/v0.50/user/run-node/github.com/cometbft/cometbft) to never expose ports that are not required to operate a node.
...
<Tip>
  The above address it set to `0.0.0.0` but it is recommended to set the tmkms
