...
Nodes should not have all ports open to the public, this is a simple way to get DDOS'd. Secondly it is recommended by [CometBFT](/v0.50/user/run-node/github.com/cometbft/cometbft) to never expose ports that are not required to operate a node.
...
```
vim $HOME/.simd/config/config.tomlpriv_validator_laddr = "tcp://0.0.0.0:26659"
```
