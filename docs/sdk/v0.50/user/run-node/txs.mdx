...
The preferred way to broadcast a transaction is to use gRPC, though using REST (via `gRPC-gateway`) or the CometBFT RPC is also posible. An overview of the differences between these methods is exposed [here](/docs/sdk/v0.50/api-reference/service-apis/grpc_rest). For this tutorial, we will only describe the gRPC method.
