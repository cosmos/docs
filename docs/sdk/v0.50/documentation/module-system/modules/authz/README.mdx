...
`x/authz` is an implementation of a Cosmos SDK module, per [ADR 30](docs/sdk/next/documentation/legacy/adr-comprehensive), that allows
...
on behalf of one account to other accounts. The design is defined in the [ADR 030](docs/sdk/next/documentation/legacy/adr-comprehensive).
...
**Note:** The authz module is different from the [auth (authentication)](docs/sdk/v0.50/documentation/module-system/modules/auth/README) module that is responsible for specifying the base transaction and account types.
