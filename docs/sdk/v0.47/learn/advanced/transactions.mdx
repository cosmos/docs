...
Transactions are comprised of metadata held in [contexts](context) and [`sdk.Msg`s](/docs/sdk/v0.47//build/building-modules/02-messages-and-queries) that trigger state changes within a module through the module's Protobuf [`Msg` service](/docs/sdk/v0.47//build/building-modules/03-msg-services).
...
Application developers create entry points to the application by creating a [command-line interface](cli), [gRPC and/or REST interface](grpc_rest), typically found in the application's `./cmd` folder. These interfaces allow users to interact with the application through command-line.
