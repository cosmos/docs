...
* **Multistore:** Every application's `BaseApp` contains a [`CommitMultiStore`](04-store#multistore) which is provided when a `Context` is created. Calling the `KVStore()` and `TransientStore()` methods allows modules to fetch their respective [`KVStore`](04-store#base-layer-kvstores) using their unique `StoreKey`.
...
* **Gas Meters:** Specifically, a [`gasMeter`](/docs/sdk/v0.47/beginner/04-gas-fees#main-gas-meter) for the transaction currently being processed using the context and a [`blockGasMeter`](/docs/sdk/v0.47/beginner/04-gas-fees#block-gas-meter) for the entire block it belongs to. Users specify how much in fees they wish to pay for the execution of their transaction; these gas meters keep track of how much [gas](/docs/sdk/v0.47/beginner/04-gas-fees) has been used in the transaction or block so far. If the gas meter runs out, execution halts.
* **CheckTx Mode:** A boolean value indicating whether a transaction should be processed in `CheckTx` or `DeliverTx` mode.
* **Min Gas Price:** The minimum [gas](/docs/sdk/v0.47/beginner/04-gas-fees) price a node is willing to take in order to include a transaction in its block. This price is a local value configured by each node individually, and should therefore **not be used in any functions used in sequences leading to state-transitions**.
...
* **Event Manager:** The event manager allows any caller with access to a `Context` to emit [`Events`](08-events). Modules may define module specific
...
For example, here is a snippet from the [`runTx`](00-baseapp#runtx-antehandler-runmsgs-posthandler) function in [`baseapp`](00-baseapp):
...
3. If the process is running in [`checkTxMode`](00-baseapp#checktx), there is no need to write the
   changes - the result is returned immediately.
4. If the process is running in [`deliverTxMode`](00-baseapp#delivertx) and the result indicates
