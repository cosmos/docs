...
A standard implementation of both these objects can be found in the [`auth/tx` module](/docs/sdk/v0.47/build/modules/auth/tx):
...
In this `Profile` example, we hardcoded `account` as a `BaseAccount`. However, there are several other types of [user accounts related to vesting](/docs/sdk/v0.47/build/modules/auth/vesting), such as `BaseVestingAccount` or `ContinuousVestingAccount`. All of these accounts are different, but they all implement the `AccountI` interface. How would you create a `Profile` that allows all these types of accounts with an `account` field that accepts an `AccountI` interface?
...
- state
- [`Msg`s](/docs/sdk/v0.47/build/building-modules/messages-and-queries#messages)
- [Query services](/docs/sdk/v0.47/build/building-modules/query-services)
- [genesis](/docs/sdk/v0.47/build/building-modules/genesis)
