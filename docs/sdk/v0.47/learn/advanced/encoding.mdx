...
[transactions](01-transactions). Transactions are defined by the application or
...
A standard implementation of both these objects can be found in the [`auth/tx` module](/docs/sdk/v0.47//build/modules/auth/2-tx):
...
The Protobuf DSL is strongly typed, which can make inserting variable-typed fields difficult. Imagine we want to create a `Profile` protobuf message that serves as a wrapper over [an account](/docs/sdk/v0.47/beginner/03-accounts):
...
In this `Profile` example, we hardcoded `account` as a `BaseAccount`. However, there are several other types of [user accounts related to vesting](/docs/sdk/v0.47//build/modules/auth/1-vesting), such as `BaseVestingAccount` or `ContinuousVestingAccount`. All of these accounts are different, but they all implement the `AccountI` interface. How would you create a `Profile` that allows all these types of accounts with an `account` field that accepts an `AccountI` interface?
...
* state
* [`Msg`s](/docs/sdk/v0.47//build/building-modules/02-messages-and-queries#messages)
* [Query services](/docs/sdk/v0.47//build/building-modules/04-query-services)
* [genesis](/docs/sdk/v0.47//build/building-modules/08-genesis)
