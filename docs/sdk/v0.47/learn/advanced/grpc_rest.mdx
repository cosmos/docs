...
* `/broadcast_tx_{aync,async,commit}`: these 3 endpoint will broadcast a transaction to other peers. CLI, gRPC and REST expose [a way to broadcast transations](/docs/sdk/v0.47/learn/advanced/transactions.mdx#broadcasting-the-transaction), but they all use these 3 CometBFT RPCs under the hood.
