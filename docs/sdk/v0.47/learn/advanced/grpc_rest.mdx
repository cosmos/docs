...
Once the gRPC server is started, you can send requests to it using a gRPC client. Some examples are given in our [Interact with the Node](/docs/sdk/v0.47/documentation/operations/interact-node#using-grpc) tutorial.
...
* `/broadcast_tx_{aync,async,commit}`: these 3 endpoint will broadcast a transaction to other peers. CLI, gRPC and REST expose [a way to broadcast transations](/docs/sdk/v0.47/documentation/protocol-development/transactions#broadcasting-the-transaction), but they all use these 3 CometBFT RPCs under the hood.
