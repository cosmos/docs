...
3. Perform non-module related *stateful* checks on the [account](/beginner/03-accounts.md). This step is mainly about checking
...
2. For each `sdk.Msg` in the transaction, route to the appropriate module's Protobuf [`Msg` service](/../build/building-modules/03-msg-services.md). Additional *stateful* checks are performed, and the branched multistore held in `deliverState`'s `context` is updated by the module's `keeper`. If the `Msg` service returns successfully, the branched multistore held in `context` is written to `deliverState` `CacheMultiStore`.
