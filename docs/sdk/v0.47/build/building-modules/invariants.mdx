...
The `InvariantRegistry` is therefore typically instantiated by instantiating the `keeper` of the `crisis` module in the [application's constructor function](/docs/sdk/v0.47//learn/beginner/overview-app#constructor-function).

`Invariant`s can be checked manually via [`message`s](./docs/sdk/v0.47/build/building-modules/messages-and-queries.mdx), but most often they are checked automatically at the end of each block. Here is an example from the `crisis` module:
