...
`Invariant`s can be checked manually via [`message`s](/docs/sdk/v0.47/build/building-modules/messages-and-queries), but most often they are checked automatically at the end of each block. Here is an example from the `crisis` module:
