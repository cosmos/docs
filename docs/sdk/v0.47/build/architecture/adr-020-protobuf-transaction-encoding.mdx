...
[ADR 019](./adr-019-protobuf-state-encoding.mdx), namely, we aim to design the
...
2. Create a `SignDoc` and serialize it using [ADR 027](./adr-027-deterministic-protobuf-serialization.mdx).
...
3. For each required signer:
   ...
   * Create a `SignDoc` and serialize it using [ADR 027](./adr-027-deterministic-protobuf-serialization.mdx).
...
in the client can be interfaces, similar to how we described in [ADR 019](./adr-019-protobuf-state-encoding.mdx),
