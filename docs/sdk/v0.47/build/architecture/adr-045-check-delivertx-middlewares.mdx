...
| RecoveryTxMiddleware    | This index recovers from panics. It replaces baseapp.runTx's panic recovery described in [ADR-022](./docs/sdk/v0.47/build/architecture/adr-022-custom-panic-handling.mdx).                                                                                                                                                                                                                                                                                                                                                       |
...
The middleware-based design builds upon the existing antehandlers design described in [ADR-010](./docs/sdk/v0.47/build/architecture/adr-010-modular-antehandler.mdx). Even though the final decision of ADR-010 was to go with the "Simple Decorators" approach, the middleware design is actually very similar to the other [Decorator Pattern](./docs/sdk/v0.47/build/architecture/adr-010-modular-antehandler.mdx#decorator-pattern) proposal, also used in [weave](https://github.com/iov-one/weave).
