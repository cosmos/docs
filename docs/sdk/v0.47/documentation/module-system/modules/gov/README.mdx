...
[ADR-037](/docs/sdk/next/documentation/legacy/adr-comprehensive#adr-037-governance-split-votes) introduces the weighted vote feature which allows a staker to split their votes into several voting options. For example, it could use 70% of its voting power to vote Yes and 30% of its voting power to vote No.
...
upon receiving txGovSubmitProposal from sender do
  ...
  initialDeposit = txGovSubmitProposal.InitialDeposit
    if (initialDeposit.Atoms <= 0)

OR (sender.AtomBalance < initialDeposit.Atoms)
    / InitialDeposit is negative or null OR sender has insufficient funds
    throw
    if (txGovSubmitProposal.Type != ProposalTypePlainText)

OR (txGovSubmitProposal.Type != ProposalTypeSoftwareUpgrade)
...
proposal.Deposits.append({
    initialDeposit, sender
})
...
throw
  ...
  proposal = load(Proposals, <txGovDeposit.ProposalID|'proposal'>) / proposal is a const key, proposalID is variable
    ...
    throw
    if (txGovDeposit.Deposit.Atoms <= 0)

OR (sender.AtomBalance < txGovDeposit.Deposit.Atoms)

OR (proposal.CurrentStatus != ProposalStatusOpen)
...
Location: off-chain as json object stored on IPFS (mirrors [group proposal](docs/sdk/v0.47/documentation/module-system/modules/group/README#metadata))
...
Location: on-chain as json within 255 character limit (mirrors [group vote](docs/sdk/v0.47/documentation/module-system/modules/group/README#metadata))
