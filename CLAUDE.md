...
This is a **multi-product monorepo** (`cosmos/docs`), not just EVM docs. Top-level product directories include `sdk/`, `cometbft/`, `evm/`, and others. The `docs/` subdirectory described in Architecture is specific to the EVM product.

## Mintlify Link Rules & Gotchas

- Links must use **absolute paths with a leading `/`** — no `.mdx` extension (e.g., `/sdk/v0.53/learn/advanced/upgrade`, not `sdk/v0.53/learn/advanced/upgrade` or `./upgrade.mdx`)
- **Missing leading `/` causes path doubling**: a link like `(sdk/v0.53/learn/advanced/upgrade)` on a page at `/sdk/v0.53/build/tooling/` resolves to `/sdk/v0.53/build/tooling/sdk/v0.53/learn/advanced/upgrade`
- Some doc files use **PascalCase or Title-Case names** (e.g., `Reactor-Api.mdx`, `Methods.mdx`). When fixing a broken link, verify the actual filename on disk — the linked slug may differ in casing from what you expect
- ABCI++ methods page in CometBFT v0.38 is `Methods.mdx` (not `abci++_methods.mdx`)