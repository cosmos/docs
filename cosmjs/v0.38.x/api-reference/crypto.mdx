...
|--------|------------|---------|
| `constructor` | `hashFunction: HashFunction`, `key: Uint8Array` | `Hmac` |
...
| Method | Parameters | Returns |
|--------|------------|---------|
| `makeKeypair` (static) | `privkey: Uint8Array` | `Promise<Secp256k1Keypair>` |
| `createSignature` (static) | `messageHash: Uint8Array`, `privkey: Uint8Array` | `Promise<ExtendedSecp256k1Signature>` |
| `verifySignature` (static) | `signature: Secp256k1Signature`, `messageHash: Uint8Array`, `pubkey: Uint8Array` | `Promise<boolean>` |
...
const signature = await Secp256k1.createSignature(hash, keypair.privkey);
const valid = await Secp256k1.verifySignature(signature, hash, keypair.pubkey);
```
