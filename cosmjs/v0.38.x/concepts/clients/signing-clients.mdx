...
| Method | Description |
|--------|-------------|
| `signAndBroadcast(address, messages, fee, memo?)` | Sign, broadcast, and wait for inclusion |
| `signAndBroadcastSync(address, messages, fee, memo?)` | Sign and broadcast, return tx hash immediately |
| `sign(address, messages, fee, memo, signerData?)` | Sign without broadcasting (returns `TxRaw`) |
