...
const client = new HttpClient("https://rpc.cosmos.network");
```
...
```typescript
const client = new HttpClient("https://rpc.cosmos.network", 10_000);
```
...
const client = new HttpBatchClient("https://rpc.cosmos.network");
```
...
const batchClient = new HttpBatchClient("https://rpc.cosmos.network", {
  batchSizeLimit: 10,
  dispatchInterval: 50,
});

const cometClient = await Comet38Client.create(batchClient);
```

> **Note:** `connectComet()` and the high-level `StargateClient.connect()` don't
> use batching by default. To enable it, create the batch client manually and
> pass it to the CometBFT client's `create()` method, then pass that to
