...
The easiest way to send transactions is using the CLI, as shown in the previous page when [interacting with a node](/sdk/v0.53/user/run-node/interact-node#using-the-cli). For example, running the following command
...
* fetch `$MY_VALIDATOR_ADDRESS` from the keyring. This is possible because the [CLI's keyring was set up](/sdk/v0.53/user/run-node/keyring) in a previous step.
...
The encoding config is set to use Protobuf, which will use `SIGN_MODE_DIRECT` by default. As per [ADR-020](https://github.com/cosmos/cosmos-sdk/blob/main/docs/architecture/adr-020-protobuf-transaction-encoding.md), each signer needs to sign the `SignerInfo`s of all other signers. This means that two steps must be performed sequentially:
...
The preferred way to broadcast a transaction is to use gRPC, though using REST (via `gRPC-gateway`) or the CometBFT RPC is also possible. An overview of the differences between these methods is exposed [here](/sdk/v0.53/learn/advanced/grpc_rest). For this tutorial, we will only describe the gRPC method.
...
CosmJS aims to build client libraries in JavaScript that can be embedded in web applications. Please see [Link](https://cosmos.github.io/cosmjs) for more information. {/* As of January 2026, CosmJS documentation is still a work in progress. */}
...
- [Run in production](/sdk/v0.53/user/run-node/run-production) for security and deployment best practices.
- [Run a testnet](/sdk/v0.53/user/run-node/run-testnet) to test your blockchain.
