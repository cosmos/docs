...
The easiest way to send transactions is using the CLI, as we have seen in the previous page when [interacting with a node](/sdk/v0.53/user/run-node/interact-node#using-the-cli). For example, running the following command
...
* fetch `$MY_VALIDATOR_ADDRESS` from the keyring. This is possible because we have [set up the CLI's keyring](/sdk/v0.53/user/run-node/keyring) in a previous step.
