...
CometBFT passes transactions to the application through an interface called the [ABCI](/cometbft/v0.38/spec/abci/Overview) (Application Blockchain Interface), which the application must implement. ABCI defines the methods through which CometBFT sends ordered transaction bytes to the application for execution and receives the corresponding results, without needing to understand the application's state or logic.
