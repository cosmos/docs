...
Messages inside a transaction are applied sequentially. The transaction succeeds only if all messages execute successfully.
...
Blocks group transactions. Transactions drive execution. Execution updates state.
...
Modules define query services using protobuf. These services are exposed over gRPC (and REST via a gateway).
