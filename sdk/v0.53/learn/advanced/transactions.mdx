...
Transactions are comprised of metadata held in [contexts](/sdk/v0.53/learn/advanced/context) and [`sdk.Msg`s](/sdk/v0.53/build/building-modules/messages-and-queries) that trigger state changes within a module through the module's Protobuf [`Msg` service](/sdk/v0.53/build/building-modules/msg-services).

When users want to interact with an application and make state changes (e.g. sending coins), they create transactions. Each of a transaction's `sdk.Msg` must be signed using the private key associated with the appropriate account(s), before the transaction is broadcasted to the network. A transaction must then be included in a block, validated, and approved by the network through the consensus process. To read more about the lifecycle of a transaction, click [here](/sdk/v0.53/learn/beginner/tx-lifecycle).
...
If you wish to learn more, please refer to [ADR-050](/sdk/v0.53/build/architecture/adr-050-sign-mode-textual).
...
To learn more about Protobuf `Msg` services and how to implement `MsgServer`, click [here](/sdk/v0.53/build/building-modules/msg-services).
...
Application developers create entry points to the application by creating a [command-line interface](/sdk/v0.53/learn/advanced/cli), [gRPC and/or REST interface](/sdk/v0.53/learn/advanced/grpc_rest), typically found in the application's `./cmd` folder. These interfaces allow users to interact with the application through command-line.

For the [command-line interface](/sdk/v0.53/build/building-modules/module-interfaces#cli), module developers create subcommands to add as children to the application top-level transaction command `TxCmd`. CLI commands actually bundle all the steps of transaction processing into one simple command: creating messages, generating transactions and broadcasting. For concrete examples, see the [Interacting with a Node](/sdk/v0.53/user/run-node/interact-node) section. An example transaction made using CLI looks like:
...
[gRPC](https://grpc.io) is the main component for the Cosmos SDK's RPC layer. Its principal usage is in the context of modules' [`Query` services](/sdk/v0.53/build/building-modules/query-services). However, the Cosmos SDK also exposes a few other module-agnostic gRPC services, one of them being the `Tx` service:
...
Examples of broadcasting and simulating a transaction are shown [here](/sdk/v0.53/user/run-node/txs#programmatically-with-go).
...
An example can be seen [here](/sdk/v0.53/user/run-node/txs#using-rest)
...
Check out the [v0.53.0 Upgrade Guide](/sdk/v0.53/build/migrations/upgrade-guide#enable-unordered-transactions-optional)
