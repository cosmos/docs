...
Under the hood the `collections.Map` will convert the key and value to bytes using the [key and value codec](README#key-and-value-codecs).
It will prepend to our bytes key the [prefix](README#prefix) and store it in the KVStore of the module.
