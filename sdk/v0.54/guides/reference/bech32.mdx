---
noindex: true
canonical: 'https://docs.cosmos.network/sdk/latest/guides/reference/bech32'
...
Every Cosmos SDK application sets its Bech32 prefixes and SLIP-44 coin type once at startup via `sdk.GetConfig()`, then seals the config so it cannot be changed at runtime. The defaults (`cosmos`, `cosmosvaloper`, etc.) are defined in [`types/config.go`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.54.x/types/config.go). Chain developers override them before the app starts:
...
- [Accounts](/sdk/v0.54/learn/concepts/accounts) - Understanding account types and management
- [Store](/sdk/v0.54/learn/concepts/store) - How addresses are used as keys in state storage
- [Transactions](/sdk/v0.54/learn/concepts/transactions) - How addresses are used in transaction signing
