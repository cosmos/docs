---
noindex: true
canonical: 'https://docs.cosmos.network/sdk/latest/modules/circuit/README'
title: 'x/circuit'
---
...
* In `CircuitBreakerDecorator` [ante handler](/sdk/v0.54/learn/concepts/baseapp#antehandler):
...
* With a [message router check](/sdk/v0.54/learn/concepts/baseapp#msg-service-router):
...
<Note>
The `CircuitBreakerDecorator` works for most use cases, but [does not check the inner messages of a transaction](/sdk/v0.54/learn/concepts/lifecycle#antehandler). This some transactions (such as `x/authz` transactions or some `x/gov` transactions) may pass the ante handler. **This does not affect the circuit breaker** as the message router check will still fail the transaction.
