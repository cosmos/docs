...
* [ABCI](/sdk/next/guides/abci/abci)
...
The nonce mempool keeps each account's transactions sorted by nonce, so they are proposed in the order the account signed them.
...
The [priority nonce mempool](https://github.com/cosmos/cosmos-sdk/blob/release/v0.55.x/types/mempool/priority_nonce_spec.md) is a mempool implementation that stores txs in a partially ordered set by 2 dimensions:
