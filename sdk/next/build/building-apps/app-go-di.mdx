...
The Cosmos SDK allows much easier wiring of an `app.go` thanks to [runtime](/sdk/next/build/building-apps/runtime) and app wiring.
Learn more about the rationale of App Wiring in [ADR-057](/sdk/next/build/architecture/adr-057-app-wiring).
...
* [What is `runtime`?](/sdk/next/build/building-apps/runtime)
* [Depinject documentation](/sdk/next/build/packages/depinject)
* [Modules depinject-ready](/sdk/next/build/building-modules/depinject)
...
1. Create the `AppConfig` variable:
   ...
   Where the `appConfig` combines the [runtime](/sdk/next/build/building-apps/runtime) configuration and the (extra) modules configuration.
...
In short `depinject` and the [`runtime` package](/sdk/next/build/building-apps/runtime) abstract the wiring of the app, and the `AppBuilder` is the place where the app is constructed. [`runtime`](/sdk/next/build/building-apps/runtime) takes care of registering the codecs, KV store, subspaces and instantiating `baseapp`.
