...
<Note>
**Synopsis**
A Protobuf Query service processes [`queries`](/sdk/next/build/building-modules/messages-and-queries). Query services are specific to the module in which they are defined, and only process `queries` defined within said module. They are called from `BaseApp`'s [`Query` method](/sdk/next/learn/advanced/baseapp#query).
</Note>
...
* [Messages and Queries](/sdk/next/build/building-modules/messages-and-queries)
