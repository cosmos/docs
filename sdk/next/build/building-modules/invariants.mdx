...
The `InvariantRegistry` is therefore typically instantiated by instantiating the `keeper` of the `crisis` module in the [application's constructor function](/sdk/next/learn/intro/sdk-app-architecture#constructor-function).

`Invariant`s can be checked manually via [`message`s](/sdk/next/build/building-modules/messages-and-queries), but most often they are checked automatically at the end of each block. Here is an example from the `crisis` module:
