...
In order to create messages that trigger state changes, end-users must create [transactions](/sdk/next/learn/concepts/transactions) that wrap and deliver the messages. A transaction command creates a transaction that includes one or more messages.
