...
The rationale behind testing can be found in [ADR-59](/sdk/next/build/architecture/adr-059-test-scopes).
...
In order to only test `x/gov`, we mock the [expected keepers](/sdk/next/build/building-modules/keeper#type-definition) and instantiate the `Keeper` with the mocked dependencies. Note that we may need to configure the mocked dependencies to return the expected values:
...
Simulations uses as well a minimal application, built with [`depinject`](/sdk/next/build/packages/depinject):
...
<Note>
**warning**
The SDK is in the process of creating its E2E tests, as defined in [ADR-59](/sdk/next/build/architecture/adr-059-test-scopes). This page will eventually be updated with better examples.
</Note>

## Learn More

Learn more about testing scope in [ADR-59](/sdk/next/build/architecture/adr-059-test-scopes).
