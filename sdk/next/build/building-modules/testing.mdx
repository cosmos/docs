...
In order to only test `x/gov`, we mock the [expected keepers](/sdk/next/learn/concepts/modules#keeper) and instantiate the `Keeper` with the mocked dependencies. Note that we may need to configure the mocked dependencies to return the expected values:
