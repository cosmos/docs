...
[`depinject`](/sdk/next/build/packages/depinject) is used to wire any module in `app.go`.
...
The App Wiring is done in `app_config.go` / `app.yaml` and `app_di.go` and is explained in detail in the [overview of `app_di.go`](/sdk/next/build/building-apps/app-go-di).
