...
The Cosmos SDK can be thought of as the Ruby-on-Rails of blockchain development. It comes with a core that provides the basic functionalities every blockchain application needs, like a [boilerplate implementation of the ABCI](/sdk/next/learn/concepts/baseapp) to communicate with the underlying consensus engine, a [`multistore`](/sdk/next/learn/advanced/store#multistore) to persist state, a [server](/sdk/next/learn/advanced/node) to form a full-node and [interfaces](/sdk/next/build/building-modules/module-interfaces) to handle queries.
...
Cosmos SDK modules can be seen as little state-machines within the state-machine. They generally define a subset of the state using one or more `KVStore`s in the [main multistore](/sdk/next/learn/advanced/store), as well as a subset of [message types](/sdk/next/build/building-modules/messages-and-queries#messages). These messages are routed by one of the main components of Cosmos SDK core, [`BaseApp`](/sdk/next/learn/concepts/baseapp), to a module Protobuf [`Msg` service](/sdk/next/build/building-modules/msg-services) that defines them.
...
* **Composability**: Cosmos SDK applications are almost always composed of multiple modules. This means developers need to carefully consider the integration of their module not only with the core of the Cosmos SDK, but also with other modules. The former is achieved by following standard design patterns outlined [here](#main-components-of-cosmos-sdk-modules), while the latter is achieved by properly exposing the store(s) of the module via the [`keeper`](/sdk/next/build/building-modules/keeper).
...
* **Capabilities**: Most modules need to read and/or write to the store(s) of other modules. However, in an open-source environment, it is possible for some modules to be malicious. That is why module developers need to carefully think not only about how their module interacts with other modules, but also about how to give access to the module's store(s). The Cosmos SDK takes a capabilities-oriented approach to inter-module security. This means that each store defined by a module is accessed by a `key`, which is held by the module's [`keeper`](/sdk/next/build/building-modules/keeper). This `keeper` defines how to access the store(s) and under what conditions. Access to the module's store(s) is done by passing a reference to the module's `keeper`.
...
* A [`keeper`](/sdk/next/build/building-modules/keeper), used to access the module's store(s) and update the state.
* A [`Msg` service](/sdk/next/build/building-modules/messages-and-queries#messages), used to process messages when they are routed to the module by [`BaseApp`](/sdk/next/learn/concepts/baseapp#message-routing) and trigger state-transitions.
* A [query service](/sdk/next/build/building-modules/query-services), used to process user queries when they are routed to the module by [`BaseApp`](/sdk/next/learn/concepts/baseapp#query-routing).
...
In addition to these components, modules implement the `AppModule` interface in order to be managed by the [`module manager`](/sdk/next/build/building-modules/module-manager).
