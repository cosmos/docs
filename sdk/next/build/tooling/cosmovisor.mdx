...
* `DAEMON_PREUPGRADE_MAX_RETRIES` (defaults to `0`). The maximum number of times to call [`pre-upgrade`](/sdk/next/build/building-apps/app-upgrade#pre-upgrade-handling) in the application after exit status of `31`. After the maximum number of retries, Cosmovisor fails the upgrade.
...
Migration plans are defined using the `x/upgrade` module and described in [In-Place Store Migrations](/sdk/next/learn/advanced/upgrade). Migrations can perform any deterministic state change.
