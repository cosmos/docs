...
* In `CircuitBreakerDecorator` [ante handler](/sdk/next/learn/advanced/baseapp#antehandler):
...
* With a [message router check](/sdk/next/learn/advanced/baseapp#msg-service-router):
...
<Note>
The `CircuitBreakerDecorator` works for most use cases, but [does not check the inner messages of a transaction](/sdk/next/learn/beginner/tx-lifecycle#antehandler). This some transactions (such as `x/authz` transactions or some `x/gov` transactions) may pass the ante handler. **This does not affect the circuit breaker** as the message router check will still fail the transaction.
