...
* ValidatorConsKeyRotation: `0x92 | ValAddress` - a validator's pending rotation, enforcing one rotation per unbonding period
* RotationLockedConsAddrIndex: `0x93 | ConsAddress` - consensus addresses locked by a rotation: the old address remains attributable for evidence, and the new address cannot be reused, until equivocation evidence for the rotated-away key can no longer be admitted
* ConsKeyRotationApplyQueue: `0x94 | height` - height-keyed queue of rotations, applied two heights after the rotation message
...
Handling the message burns the `KeyRotationFee` param from the operator account and enqueues the rotation. The new key enters the CometBFT validator set two heights after the message executes. The rotation history is retained for the unbonding period so that evidence against the old key still slashes the validator.
