...
Pending and historical consensus key rotations are tracked in four stores:

* ConsKeyRotationQueue: `0x91 | format(maturityTime) | ValAddress` - rotations awaiting the end of their unbonding period, at which point the old consensus address unlocks and the entry is pruned
* ValidatorConsKeyRotation: `0x92 | ValAddress` - a validator's pending rotation, enforcing one rotation per unbonding period
* RotationLockedConsAddrIndex: `0x93 | ConsAddress` - consensus addresses locked by a rotation: the old address remains attributable for evidence, and the new address cannot be reused, until maturity
