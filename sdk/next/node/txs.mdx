...
The easiest way to send transactions is using the CLI, as shown in the previous page when [interacting with a node](/sdk/next/node/interact-node#using-the-cli). For example, running the following command
...
* fetch `$MY_VALIDATOR_ADDRESS` from the keyring. This is possible because the [CLI's keyring was set up](/sdk/next/node/keyring) in a previous step.
...
For what each field in that JSON means, see [Sending Transactions](/sdk/next/api-reference/transactions#the-envelope) in the API reference.
...
The preferred way to broadcast a transaction is to use gRPC, though using REST (via `gRPC-gateway`) or the CometBFT RPC is also possible. An overview of the differences between these methods is exposed [here](/sdk/next/learn/concepts/cli-grpc-rest). For this tutorial, we will only describe the gRPC method.
...
- [Run in production](/sdk/next/node/run-production) for security and deployment best practices.
- [Run a testnet](/sdk/next/node/run-testnet) to test your blockchain.
