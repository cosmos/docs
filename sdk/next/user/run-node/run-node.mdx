...
* [Prerequisites](/sdk/next/user/run-node/prerequisites) - Set up Go and build the `simd` binary
* [Anatomy of a Cosmos SDK Application](/sdk/next/learn/intro/sdk-app-architecture)
* [Setting up the keyring](/sdk/next/user/run-node/keyring)
...
Earlier in this tutorial, you [created an account in the keyring](/sdk/next/user/run-node/keyring#create-a-key) named `my_validator` under the `test` keyring backend.
...
Recall that `$MY_VALIDATOR_ADDRESS` is a variable that holds the address of the `my_validator` key in the [keyring](/sdk/next/user/run-node/keyring#create-a-key). Also note that the tokens in the Cosmos SDK have the `{amount}{denom}` format: `amount` is an 18-digit-precision decimal number, and `denom` is the unique token identifier with its denomination key (e.g., `atom` or `uatom`). Here, `stake` tokens are granted, as `stake` is the token identifier used for staking in [`simapp`](https://github.com/cosmos/cosmos-sdk/tree/main/simapp). For your own chain with its own staking denom, that token identifier should be used instead.
...
* `config.toml`: used to configure the CometBFT, learn more on [CometBFT's documentation](/cometbft/v0.38/docs/core/configuration),
...
The `start` command (defined in [`server/start.go`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.53.x/server/start.go)) boots up the full-node in the following sequence:
...
The naive way would be to run the same commands again in separate terminal windows. This is possible. However, [Docker Compose](https://docs.docker.com/compose/) can be leveraged to run a localnet. If you need inspiration on how to set up your own localnet with Docker Compose, refer to the Cosmos SDK's [`docker-compose.yml`](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/docker-compose.yml).
...
State sync is the act in which a node syncs the latest or close to the latest state of a blockchain. This is useful for users who don't want to sync all the blocks in history. Read more in [CometBFT documentation](/cometbft/v0.38/docs/core/state-sync).
...
1. As mentioned in the [state sync documentation](/cometbft/v0.38/docs/core/state-sync), one must set a height and hash in the config.toml along with a few RPC servers (the aforementioned link has instructions on how to do this).
...
- [Interact with the node](/sdk/next/user/run-node/interact-node) to send transactions and query state
- [Generate and sign transactions](/sdk/next/user/run-node/txs) to learn advanced transaction workflows