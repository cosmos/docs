...
The easiest way to send transactions is using the CLI, as shown in the previous page when [interacting with a node](/sdk/next/user/run-node/interact-node#using-the-cli). For example, running the following command
...
* fetch `$MY_VALIDATOR_ADDRESS` from the keyring. This is possible because the [CLI's keyring was set up](/sdk/next/user/run-node/keyring) in a previous step.
...
- [Run in production](/sdk/next/user/run-node/run-production) for security and deployment best practices.
- [Run a testnet](/sdk/next/user/run-node/run-testnet) to test your blockchain.
