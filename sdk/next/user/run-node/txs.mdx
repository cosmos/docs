...
The preferred way to broadcast a transaction is to use gRPC, though using REST (via `gRPC-gateway`) or the CometBFT RPC is also possible. An overview of the differences between these methods is exposed [here](/sdk/next/learn/advanced/grpc_rest). For this tutorial, we will only describe the gRPC method.
