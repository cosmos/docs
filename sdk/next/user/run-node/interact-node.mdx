...
* [Running a Node](/sdk/next/user/run-node/run-node)
...
The list of all available REST endpoints is available as a Swagger specification file, which can be viewed at `localhost:1317/swagger`. Make sure that the `api.swagger` field is set to true in your [`app.toml`](/sdk/next/user/run-node/run-node#configuring-the-node-using-apptoml-and-configtoml) file.
...
- [Generate and sign transactions](/sdk/next/user/run-node/txs) to learn manual transaction workflows
