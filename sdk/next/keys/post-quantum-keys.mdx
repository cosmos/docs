...
A single ML-DSA signature is about 3,300 bytes, over 50 times an ed25519 signature. Every block stores one per validator, which drives the block-data growth shown below.
...
Any counterparty chains that verify an ML-DSA-enabled chain with an `07-tendermint` light client must be upgraded to CometBFT v0.40 or later. An older client fails as soon as the first ML-DSA validator joins the set, so every counterparty must upgrade to v0.40 before any validator can rotate to ML-DSA.
