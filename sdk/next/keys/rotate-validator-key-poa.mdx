...
- A running PoA validator you operate, with the chain's binary on your PATH and a secondary machine or scratch directory for the new key. PoA validators are set in genesis under `app_state.poa`.
  {/* app_state.poa shape: "poa": { "params": { "admin": "cosmos1..." }, "validators": [{ "pub_key": { "@type": "/cosmos.crypto.ed25519.PubKey", "key": "<pub_key.value from priv_validator_key.json>" }, "power": "10000000", "metadata": { "moniker": "mynode", "operator_address": "cosmos1..." } }] } */}
- For an ML-DSA rotation, `ml_dsa_65` is in the chain's consensus params. See [Enable ML-DSA keys](/sdk/next/keys/enable-ml-dsa-keys).
