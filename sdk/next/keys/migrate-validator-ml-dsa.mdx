...
- All prerequisites of the rotation procedure: [jq](https://jqlang.org/) and [curl](https://curl.se/), no rotation in the current unbonding period, and fee funds on the operator account. See [Rotate a consensus key, Staking](/sdk/next/keys/rotate-validator-key).
...
Follow [Rotate a consensus key, Staking](/sdk/next/keys/rotate-validator-key), replacing the `simd init` command in its step 1 with one that adds  `--consensus-key-algo ml_dsa_65` to create an ML-DSA key:
...
The cutover timing is unchanged: keep the node on the old key until the validator set switches, then swap the key file in place, exactly as the guide's [steps 3 and 4](/sdk/next/keys/rotate-validator-key-poa#3-wait-for-the-validator-set-to-switch) describe.
...
- Check which key types the chain allows and which the validator set is running. See [Enable ML-DSA keys](/sdk/next/keys/enable-ml-dsa-keys).
