...
<Warning>Before rotating any validator to ML-DSA, confirm every counterparty chain that verifies this chain over IBC runs CometBFT v0.40 or later. An older `07-tendermint` light client cannot verify ML-DSA consensus signatures. See [IBC considerations](/sdk/next/keys/post-quantum-keys#ibc-considerations).</Warning>
...
If `validator.pub_key_types` does not include `ml_dsa_65`, the rotation is rejected. To add the type, see [Enable ML-DSA keys](/sdk/next/keys/enable-ml-dsa-keys).
...
And pass `ml_dsa_65` instead of `ed25519` as the key type when submitting. The ML-DSA public key is large enough that the default gas limit runs out, so add `--gas auto`:
...
*/}
{/* TODO(MATT): this ML-DSA PoA rotation command needed a fix; https://github.com/cosmos/cosmos-sdk/pull/26614 fixes it (verified on that PR branch). Test from that PR, and re-confirm it works after it merges. */}
...
<Note>The consensus key this rotation installs is a raw keypair with no mnemonic. If you also hold an ML-DSA user account, do not derive it from a mnemonic already used for a secp256k1 account, since at the same derivation path the two keys share one secret. See [Create an ML-DSA account](/sdk/next/keys/create-ml-dsa-account).</Note>
