...
{/* TODO(FLAG Eric): document the supported keygen path for an ML-DSA priv_validator_key.json. comet init and gen-validator generate ed25519 only in the current checkouts; the release demo minted ML-DSA keys, so tooling exists but is not yet in a tagged release. please update this when the fix is in to generate an ML-DSA key with init and testnet commands.*/}
