---
noindex: true
title: "Create an ML-DSA user account"
...
Raise the gas limit for this transfer. The account's first transaction writes its public key to state, and an ML-DSA public key is large enough that the default limit of 200000 runs out of gas. `--gas auto` sizes the limit to fit; a fixed `--gas 400000` also works. Later transactions from the account are smaller and fit within the default.

A successful send from `pq-account` means the chain accepted an ML-DSA signature for the account. Drain and retire the old account at your own pace. Nothing on the chain depends on it.

## What can go wrong

- The key type is rejected at creation: the binary predates ML-DSA support. The chain and its CLI must run Cosmos SDK 0.55 or later.
- An EVM transaction from the account fails: this is expected. ML-DSA accounts cannot sign EVM transactions. See [Post-quantum keys](/sdk/next/keys/post-quantum-keys).
