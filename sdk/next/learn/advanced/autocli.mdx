...
3. Use the `autocli.AppOptions` struct to specify the modules you defined.
...
AutoCLI automatically skips unsupported commands when [`cosmos_proto.method_added_in` protobuf annotation](/sdk/next/build/building-modules/protobuf-annotations) is present.
...
For AutoCLI-generated commands (and hand-written commands) to work correctly — signing transactions, querying the chain, reading configuration — the root command must set up the `client.Context` and `server.Context` in a `PersistentPreRun` function. This runs before every subcommand and makes both contexts available to all child commands.
...
### Registering with AutoCLI

To use manual commands alongside AutoCLI-generated ones, set `EnhanceCustomCommand: true` in `AutoCLIOptions()`. AutoCLI will add only the commands that are not already registered:

```go
func (am AppModule) AutoCLIOptions() *autocliv1.ModuleOptions {
    return &autocliv1.ModuleOptions{
        Tx: &autocliv1.ServiceCommandDescriptor{
            Service:              mymodule.Msg_ServiceDesc.ServiceName,
            EnhanceCustomCommand: true, // keep manually registered tx commands
        },
        Query: &autocliv1.ServiceCommandDescriptor{
            Service:              mymodule.Query_ServiceDesc.ServiceName,
            EnhanceCustomCommand: true, // keep manually registered query commands
        },
    }
}
```

Without `EnhanceCustomCommand: true`, AutoCLI skips command generation for any service that already has commands registered via `GetTxCmd()` or `GetQueryCmd()`.
...
For more information on `hubl`, including how to configure a new chain and query a module, see the [Hubl documentation](/sdk/next/build/tooling/confix).
