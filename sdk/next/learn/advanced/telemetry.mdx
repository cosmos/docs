...
<!-- TODO: This page needs a complete rewrite for v0.54. The old go-metrics based telemetry system is deprecated.
     New documentation should cover:
     - OpenTelemetry declarative configuration (https://pkg.go.dev/go.opentelemetry.io/contrib/otelconf)
     - BaseApp instrumentation
     - DiskIO telemetry via gopsutil
     - Migration guide from go-metrics to OpenTelemetry
-->

<Warning>
As of v0.54, the telemetry package based on `hashicorp/go-metrics` is deprecated. Applications should migrate to [OpenTelemetry](https://opentelemetry.io/) using the official [OpenTelemetry Go API](https://pkg.go.dev/go.opentelemetry.io/otel). The Cosmos SDK now supports automatic OpenTelemetry configuration via [declarative configuration](https://pkg.go.dev/go.opentelemetry.io/contrib/otelconf) and includes built-in instrumentation for BaseApp.
</Warning>
...
The following documentation describes the legacy telemetry system and will be updated in a future release.
