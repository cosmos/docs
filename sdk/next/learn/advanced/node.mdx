...
* [Anatomy of an SDK application](/sdk/next/learn/intro/sdk-app-architecture)
...
* First, an [`encodingCodec`](/sdk/next/learn/advanced/encoding) is instantiated for the application.
* Then, the `config` is retrieved and config parameters are set. This mainly involves setting the Bech32 prefixes for [addresses](/sdk/next/learn/concepts/accounts#addresses).
...
In practice, the [constructor of the application](/sdk/next/learn/intro/sdk-app-architecture#constructor-function) is passed as the `appCreator`.
...
To discover how to concretely run a node and interact with it, please refer to our [Running a Node, API and CLI](/sdk/next/user/run-node/run-node) guide.
