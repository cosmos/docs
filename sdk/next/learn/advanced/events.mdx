...
*Typed Events* are Protobuf-defined [messages](/sdk/next/build/architecture/adr-032-typed-events) used by the Cosmos SDK
...
They are triggered from the module's Protobuf [`Msg` service](/sdk/next/build/building-modules/msg-services)
...
* [`BeginBlock`](/sdk/next/learn/concepts/baseapp#beginblock)
* [`EndBlock`](/sdk/next/learn/concepts/baseapp#endblock)
* [`CheckTx`](/sdk/next/learn/concepts/baseapp#checktx)
* [`Transaction Execution`](/sdk/next/learn/concepts/baseapp#transactionexecution)
...
| `message.action='/cosmos.bank.v1beta1.Msg/Send'` | Query all transactions containing an x/bank `Send` [Service `Msg`](/sdk/next/build/building-modules/msg-services). Note the `'`s around the value. |
| `message.module='bank'`                          | Query all transactions containing messages from the x/bank module. Note the `'`s around the value.                                                          |
| `create_validator.validator='cosmosval1...'`     | x/staking-specific Event, see [x/staking SPEC](/sdk/next/build/modules/staking/README).                                                              |
...
`Handler` and in each `BeginBlock`/`EndBlock` handler. The `EventManagerI` interface also provides `OverrideEvents(Events)`, which replaces all currently tracked events with the provided set. The `EventManager` is accessed via
the [`Context`](/sdk/next/learn/concepts/context-gas-events), where Events should be already registered and emitted like this:
...
The `EventManager` is accessed via the [`Context`](/sdk/next/learn/concepts/context-gas-events).

See the [`Msg` services](/sdk/next/build/building-modules/msg-services) concept doc for a more detailed
...
where `ownerAddress` is an address following the [`AccAddress`](/sdk/next/learn/concepts/accounts#addresses) format.
