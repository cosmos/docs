...
The Cosmos SDK provides a layered testing approach that mirrors the architecture of the framework itself. Tests are organized into three levels, each testing a progressively larger slice of the application. The examples on this page come from the counter module tutorial.
...
The Cosmos SDK simulation framework drives this through `simsx`. The counter module defines a message factory that generates random `MsgAddRequest` messages:
...
The `testutil` package (`github.com/cosmos/cosmos-sdk/testutil`) provides helpers for constructing in-memory contexts for unit tests:
...
- `baseapp.NewQueryServerTestHelper` creates a gRPC server backed by an in-memory connection, allowing keeper tests to exercise the query server via a real gRPC client.
...
`simsx` (`cosmossdk.io/x/simulation/simsx` or `github.com/cosmos/cosmos-sdk/testutil/simsx`) is the simulation execution framework. It provides:

- `SimMsgFactoryFn`: the interface for message factories. Each factory selects random accounts and parameters, constructs a message, and returns it for execution.
...
`simd` is the reference simulation binary provided by the Cosmos SDK. It is a fully configured simapp (`simapp`) compiled as a standalone binary, used to run simulations against the SDK's own module set without setting up a custom chain. For a custom chain like the example app, you use your own binary with the `sims` build tag.
...
The SDK provides a telemetry package built around Prometheus and OpenTelemetry. Modules can emit counters, gauges, and histograms from keeper methods to expose runtime behavior for monitoring.
