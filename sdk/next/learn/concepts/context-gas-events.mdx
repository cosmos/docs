...
The `Context` struct is defined in [`types/context.go`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.53.x/types/context.go): {/* cosmos/cosmos-sdk types/context.go:41 — type Context struct { ... } */}
...
`ctx.IsCheckTx()` returns true when the context is being used for mempool validation rather than block execution. {/* cosmos/cosmos-sdk types/context.go:99 — func (c Context) IsCheckTx() bool { return c.checkTx } */} For finer-grained branching, `ctx.ExecMode()` returns the precise execution mode: `ExecModeCheck`, `ExecModeReCheck`, `ExecModeSimulate`, `ExecModePrepareProposal`, `ExecModeProcessProposal`, `ExecModeFinalize`, and others (see [`types/context.go`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.53.x/types/context.go#L19) for more details). {/* cosmos/cosmos-sdk types/context.go:102 — func (c Context) ExecMode() ExecMode { return c.execMode } */} Modules that need to behave differently during simulation or proposal handling use `ExecMode()` instead of `IsCheckTx()`.
...
Modules that need to attempt a sub-operation and revert it on failure can call [`ctx.CacheContext()`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.53.x/types/context.go#L410), which returns a branched copy of the context and a `writeCache` function. {/* cosmos/cosmos-sdk types/context.go:410 — func (c Context) CacheContext() (cc Context, writeCache func()) — branches the multistore and creates a fresh EventManager; writeCache flushes both on success */} All state changes in the sub-operation go into the branch. Calling `writeCache()` flushes them to the parent context; not calling it discards them atomically.
...
The [`GasMeter`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.53.x/store/types/gas.go#L41) interface provides two key methods: {/* cosmos/cosmos-sdk store/types/gas.go:41 — type GasMeter interface { GasConsumed() Gas; ConsumeGas(amount Gas, descriptor string); ... } */}
...
Modules emit events through the [`EventManager`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.53.x/types/events.go#L24), which is attached to the context: {/* cosmos/cosmos-sdk types/context.go:104 — func (c Context) EventManager() *EventManager { return c.eventManager } */}
