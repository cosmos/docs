...
An account does not execute logic itself; instead, it authorizes transactions. On standard ordered transactions, the account's sequence number is used for replay protection during transaction processing. Balance changes are handled by the modules that process the transaction's messages.
...
Transactions in the Cosmos SDK can be ordered or unordered.Ordered transactions are the default and are processed in the order they are submitted. 

For standard ordered transactions, each on-chain account tracks a sequence number. The sequence number starts at zero for a newly created account and is checked as part of transaction processing so that old signed transactions cannot be replayed.
