...
Each module's KVStore is backed by a [`CommitKVStore`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.54.x/store/iavl/store.go#L31). [See the store spec for more details.](/sdk/next/build/spec/store/store)
...
The [`CommitKVStore`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.54.x/store/iavl/store.go#L31) is the main persistent store backed by IAVL. It persists across blocks, produces versioned commits, and contributes to the app hash.
...
For more information on stores, pruning strategies, and store configuration, see the [store spec](/sdk/next/build/spec/store/store). For the full store interface definitions, see [`store/types/store.go`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.54.x/store/types/store.go) in the SDK source.
