...
See [Encoding and Protobuf](/sdk/next/learn/concepts/encoding) for details on how modules serialize data into bytes.
...
Each module's KVStore is backed by a [`CommitKVStore`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.54.x/store/iavl/store.go#L36). [See the store spec for more details.](/sdk/next/guides/state/store)
...
Because state access is the dominant cost of transaction execution, the SDK charges gas at the store layer so that expensive reads and writes are reflected in transaction fees. Every read and write of a KVStore costs gas, and expensive operations naturally cost more. [Execution Context, Gas, and Events](/sdk/next/learn/concepts/context-gas-events) explains how gas metering works at runtime.
...
```
For the keeper's role within a module, see [Keeper](/sdk/next/learn/concepts/modules#keeper).
...
For more information on stores, pruning strategies, and store configuration, see the [store spec](/sdk/next/guides/state/store). For the full store interface definitions, see [`store/types/store.go`](https://github.com/cosmos/cosmos-sdk/blob/release/v0.54.x/store/types/store.go) in the SDK source.
