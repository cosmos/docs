...
`SetOrderExportGenesis` controls the order modules serialize their state when the chain is exported to a genesis file (for hard forks or snapshot-based testnets). It can differ from `SetOrderInitGenesis` but in most chains the same order works for both.
...
A PostHandler can also be registered with `app.SetPostHandler`. It runs after all messages in a transaction execute successfully, in the same state branch, and is reverted if it fails. The SDK's default PostHandler chain is minimal; chains use it for features like transaction tips. For a deeper look at how the AnteHandler fits into transaction execution, see [AnteHandler](/sdk/next/learn/concepts/baseapp#antehandler).
