...
{/* TODO: link to BaseApp page for full explanation of these handlers */}
...
A PostHandler can also be registered with `app.SetPostHandler`. It runs after all messages in a transaction execute successfully, in the same state branch, and is reverted if it fails. The SDK's default PostHandler chain is minimal; chains use it for features like transaction tips.
