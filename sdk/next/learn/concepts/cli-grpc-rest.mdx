...
The CLI is the primary tool for developers and operators interacting with a chain from the terminal. Most Cosmos SDK chains ship a single binary (for example, `exampled` or `simd`) that acts as both the server process and the CLI client.
