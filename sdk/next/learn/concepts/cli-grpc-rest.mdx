...
|---|---|---|---|
| **CLI** | — | Development, testing, and node operations | Not suitable for programmatic use or automation |
...
Gas limits the computational work a transaction can perform. The full gas model is explained in [Execution Context, Gas, and Events](/sdk/next/learn/concepts/gas).
...
gRPC is the primary programmatic interface for interacting with a Cosmos SDK chain. It uses Protocol Buffers to define strongly typed request and response structures and supports generated clients for many programming languages.
...
To generate Swagger documentation for your own custom modules, see [Protobuf Tooling](/sdk/next/build/tooling/protobuf).
...
The Cosmos SDK publishes its protobuf definitions and gRPC service documentation at [buf.build/cosmos/cosmos-sdk](https://buf.build/cosmos/cosmos-sdk). The protobuf definitions are the source of truth. gRPC services and REST endpoints are both derived from them.

For a live node, the Swagger UI at `http://localhost:1317/swagger/` documents all available REST endpoints for that specific chain.
