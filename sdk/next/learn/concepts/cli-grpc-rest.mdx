...
To learn how to run a local node and use the CLI, see [Run a Local Node](/sdk/next/node/prerequisites).
...
Every query method and transaction message of the standard modules is documented in the [API reference](/sdk/next/api-reference/index), with a field table and a runnable example for each.
...
For more usage examples, see [Interact with the Node](/sdk/next/node/interact-node#using-grpc).
...
The generated routes for the standard modules, with a request playground for each, are in the [REST reference](/sdk/next/api-reference/index).
...
To generate Swagger documentation for your own custom modules, see the [`proto-swagger-gen` script](https://github.com/cosmos/cosmos-sdk/blob/release/v0.55.x/scripts/protoc-swagger-gen.sh) in the Cosmos SDK.
