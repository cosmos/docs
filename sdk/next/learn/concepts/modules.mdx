...
To update params, a governance proposal submits a [`MsgUpdateParams`](/sdk/next/modules/consensus/README#msgupdateparams) message. The `MsgServer` checks that the caller is the designated authority address (usually the governance module account) before writing the new values to the store. See [Message execution (`MsgServer`)](#message-execution-msgserver) for a code example of this pattern.

The authority address is set at keeper construction time in [`app.go`](/sdk/next/learn/concepts/app-go#initializing-keepers).

For chain-level consensus parameters, the [`x/consensus`](/sdk/next/modules/consensus) module manages them centrally; it also supports [`AuthorityParams`](/sdk/next/modules/consensus/README#authorityparams), which lets governance update the authority address on-chain without a software upgrade.
...
| Module | What it does |
| --- | --- |
| [x/auth](/sdk/next/modules/auth) | Accounts, authentication, and transaction signing |
| [x/bank](/sdk/next/modules/bank) | Token balances and transfers |
| [x/staking](/sdk/next/modules/staking) | Validator set and delegations |
| [x/gov](/sdk/next/modules/gov) | On-chain governance and proposals |
| [x/distribution](/sdk/next/modules/distribution) | Staking reward distribution |
| [x/slashing](/sdk/next/modules/slashing) | Validator penalty enforcement |
| [x/mint](/sdk/next/modules/mint) | Token issuance |
| [x/evidence](/sdk/next/modules/evidence) | Submission and handling of validator misbehavior |
| [x/upgrade](/sdk/next/modules/upgrade) | Coordinated chain upgrades |
| [x/authz](/sdk/next/modules/authz) | Delegated message authorization |
| [x/feegrant](/sdk/next/modules/feegrant) | Fee allowances between accounts |
| [x/consensus](/sdk/next/modules/consensus) | On-chain management of CometBFT consensus params |

The above are just a few of the modules available. Applications can include any subset of these modules and can define entirely new custom modules. For the full list, see [List of Modules](/sdk/next/modules/modules).
...
Before writing a module, review [Module Design Considerations](/sdk/next/guides/module-design/module-design-considerations) for guidance on state structure, message surface, inter-module dependencies, and upgrade planning. When you are ready to build, follow the [Build a Module tutorial](/sdk/next/tutorials/example/00-overview) for a step-by-step walkthrough.
