...
The `service Msg` block is referred to as the `Msg service` in the Cosmos SDK. The protobuf compiler generates a `MsgServer` interface from it, which the module implements in Go. `BaseApp` routes each incoming message to the correct implementation by its fully-qualified method name (e.g., `/cosmos.bank.v1beta1.MsgSend`).
...
{/* todo: link to Build a Module tutorial */}
The following examples are from the `x/counter` tutorial, which walks you through building a module that lets accounts increment a shared counter.
...
A module's **keeper** is its state access layer. It owns the module's KV store and provides typed methods for reading and writing state. The store fields are unexported, so nothing outside the `keeper` package can access them directly.
...
The following is an example of the `x/counter` keeper from the counter module tutorial, which holds a single state item: {/* todo: link to counter tutorial */}
...
`counter` uses `collections.Item[uint64]`, a typed single-value entry backed by the module's KV store. `AddCount` reads the current value, increments it, and writes it back. All state access goes through the keeper: nothing outside the `keeper` package can reach `counter` directly. For details on how the keeper opens its store from the context it receives, see [Execution Context](/sdk/next/learn/concepts/context-gas-events).
...
{/* todo: link to counter tutorial section on expected keepers */}
...
```go
func (a AppModule) PreBlock(ctx context.Context) error {
    // runs before BeginBlock; used for consensus parameter changes
    return nil
}
...
Implementing these methods satisfies the corresponding interface (`HasPreBlocker`, `HasBeginBlocker`, `HasEndBlocker`). The module also needs to be registered with the `ModuleManager` in `app.go` for the hooks to be called. That wiring is covered in the `app.go` section.

{/* todo: link to baseapp#module-manager — ModuleManager coordinates hook invocation order across all modules */}

{/* todo: link to app.go section */}

{/* todo: link to tutorial block hooks section */}
