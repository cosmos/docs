...
- The **`Keeper`** is the module’s state access layer: it defines the storage schema and provides the sanctioned APIs for reading and writing state. 
...
A **keeper** holds references to the module’s KV store (or collections schema) and any external keepers it depends on. It provides typed methods for reading and writing state and defines the module’s storage layout.
