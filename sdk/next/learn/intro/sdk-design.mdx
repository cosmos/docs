...
The Cosmos SDK is a framework that facilitates the development of secure state machines on top of CometBFT. At its core, the Cosmos SDK is a boilerplate implementation of the [ABCI](/sdk/next/learn/intro/sdk-app-architecture) in Golang. It comes with a [`multistore`](/sdk/next/learn/advanced/store) to persist data and a [`router`](/sdk/next/learn/advanced/baseapp) to handle transactions.
...
For more information on `baseapp`, please click [here](/sdk/next/learn/advanced/baseapp).

## Multistore

The Cosmos SDK provides a [`multistore`](/sdk/next/learn/advanced/store#multistore) for persisting state. The multistore allows developers to declare any number of [`KVStores`](/sdk/next/learn/advanced/store#base-layer-kvstores). These `KVStores` only accept the `[]byte` type as a value, and therefore any custom structure needs to be marshaled using [a codec](/sdk/next/learn/advanced/encoding) before being stored.

The multistore abstraction is used to divide the state into distinct compartments, each managed by its own module. For more on the multistore, click [here](/sdk/next/learn/advanced/store#multistore).
...
In addition to the already existing modules in `x/`, which anyone can use in their app, the Cosmos SDK lets you build your own custom modules. For more information on Cosmos modules, visit the [module section](/sdk/next/build/building-modules/intro).
