...
{/* TODO(ERIC): the mldsa65 file-backend flow is source-verified (signing/file/mldsa65.go) but blocked on ML-DSA keygen tooling for a real run; test when Eric's keygen PR lands and add the manual verification comment. */}
