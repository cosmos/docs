...
- New validator: register it with the new key's consensus public key using `gentx --pubkey` (or the `pubkey` field of `create-validator`'s validator.json). Read the public key from the custodian itself, since `gentx` runs before the chain exists and there is no node to query: for PKCS#11, `pkcs11-tool --read-object --type pubkey`, then base64 the raw key bytes; for AWS KMS, `aws kms get-public-key`.
