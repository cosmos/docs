...
<Warning>AWS KMS has a limit to the size of the data it will sign. This backend sends the raw consensus message to AWS KMS, which rejects anything larger than 4096 bytes. Be careful with features that make that message large, such as vote extensions.</Warning>
