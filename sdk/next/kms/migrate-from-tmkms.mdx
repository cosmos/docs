...
This guide moves a validator's signing from TMKMS to Cosmos-KMS, the recommended remote signer going forward. The migration translates the config, gets the key into a Cosmos-KMS backend, and cuts over with exactly one signer alive at every moment. For what Cosmos-KMS is and how it relates to TMKMS, see [Cosmos-KMS and remote signing](/sdk/next/kms/remote-signing).
...
A complete `kms.yaml` for a single validator on the file backend:
...
The `keys` block shown is the file backend; it differs per backend, which step 2 fills in. The [configuration reference](/sdk/next/kms/configuration-reference) lists every field.
...
Ledger-held consensus keys have no path today. The TMKMS Ledger signer is not implemented in Cosmos-KMS and is out of scope for this release. If you need it, open a [feature request](https://github.com/cosmos/kms/issues) so it can be evaluated by demand.

## 3. Stop TMKMS

Stop TMKMS and confirm the process is gone. The validator misses blocks from this moment, which is expected and safe.
...
TMKMS and Cosmos-KMS both track the last signed height, round, and step per chain, and the protection only works if the new signer starts at or above the old signer's high-water mark. The two store this state differently, so it needs translating rather than copying.

Point the command at the TMKMS state file named by `state_file` in your `[[chain]]` block; `tmkms_state.json` is a placeholder. It writes the translated state into the Cosmos-KMS location:
...
The `step + 1` is deliberate: TMKMS numbers the signing steps 0, 1, 2 while CometBFT numbers them 1, 2, 3, so the translation remaps them. Do not remove it.
...
Confirm the chain includes the validator's signatures again and the new signer's state file advances.
...
- `dial tcp: lookup node-id@host:port no such host`: the `<node-id>@` prefix was left in `validators[].addr`. Drop it and keep only host and port.
