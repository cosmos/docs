...
If you still have the validator's original `priv_validator_key.json` (TMKMS softsign was imported from it), point `key_file` at that file directly. No conversion is needed. That file lives on the node host, so copy it to the signer's home or give `key_file` an absolute path; a bare filename resolves against the signer's `--home`.
