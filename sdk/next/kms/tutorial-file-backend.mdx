...
<Warning>Pass `--allow-fresh-state` only on a first start, and only for a chain the key has never signed on. Once the state file exists the flag does nothing, so leaving it in a service definition is not immediately visible, but it means a deleted or truncated state file silently resets the double-sign floor to zero instead of stopping the signer. Later starts in this tutorial use the bare command.</Warning>
