---
noindex: true
title: "Run cosmos-kms with the file backend"
...
- A chain binary at Cosmos SDK 0.55 or later. The tutorial uses `simd`, built with `make install` in the cosmos-sdk repo.
...
The command prints `initialized kms in /Users/you/.kms-demo`. Pass the `--home` flag on every kms command. Without it, kms uses the current directory.
...
The signer logs `kms started` and dials the node. The node is not running yet, so kms logs `dial failed; backing off` and keeps retrying. That is expected. Leave it running.
...
<Note>The node cannot start consensus without its signer. Start it while no signer is reachable and `simd start` exits with `can't get pubkey: ... endpoint connection timed out` instead of waiting. If the node exits this way, leave kms running and rerun `simd start`; the signer keeps retrying and connects.</Note>

## 9. Prove the signer is doing the signing

Stop the kms process with Ctrl-C and watch the node's logs. Block production stalls because the validator can no longer sign. Start kms again:
...
A validator whose consensus key lives outside the node. The node handles consensus and networking. The kms process holds the key and signs, and the double-sign state file travels with the signer. The file backend keeps this tutorial self-contained, but it holds the key in plaintext on disk and is not production custody. The production version of this setup swaps one config block to move the key into an HSM or AWS KMS.
