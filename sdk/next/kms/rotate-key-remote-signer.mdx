...
<Note>For a post-quantum `mldsa65` target on the file backend, add `--consensus-key-algo ml_dsa_65` to the `simd init` command above and set `algorithm: mldsa65` in the signer's `keys` block. The PKCS#11 and AWS KMS backends also register `mldsa65`. See [Configure a signing backend](/sdk/next/kms/configure-backend) for more info.</Note>
