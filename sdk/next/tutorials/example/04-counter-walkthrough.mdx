...
[Telemetry](/sdk/next/guides/testing/telemetry) records how often the counter is updated so you can observe module activity in an OpenTelemetry-compatible system.
...
[AutoCLI](/sdk/next/guides/tooling/autocli) exposes the module's queries and transactions as CLI commands. The full module example keeps the same basic AutoCLI setup as the minimal module and adds the recommended setting for custom command integration.
...
[Simulation](/sdk/next/guides/testing/simulator) lets the SDK generate randomized transactions against the module during fuzz-style testing.
