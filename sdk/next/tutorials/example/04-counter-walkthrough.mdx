...
[Telemetry](/sdk/next/user/run-node/telemetry) records how often the counter is updated so you can observe module activity in an OpenTelemetry-compatible system.
...
[AutoCLI](/sdk/next/build/building-modules/autocli) exposes the module's queries and transactions as CLI commands. The full module example keeps the same basic AutoCLI setup as the minimal module and adds the recommended setting for custom command integration.
