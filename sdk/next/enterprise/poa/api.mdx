---
title: "API Reference"
description: "Complete API reference for PoA module gRPC queries and transactions"
---
...
The Proof of Authority (PoA) permissioned consensus module provides a governance mechanism for managing validators in a Cosmos SDK blockchain. Unlike traditional Proof of Stake, PoA allows a designated admin to control validator set membership and voting power distribution.
...
Replace a validator's consensus public key in place.

**gRPC:** `cosmos.poa.v1.Msg/RotateConsPubKey`

{/* TODO(MATT): provisional against cosmos/cosmos-sdk#26590; re-verify this section at merge. */}

**Message:**
```protobuf
message MsgRotateConsPubKey {
  string sender = 1;                       // Signer: the validator's operator or the chain admin
  string validator_address = 2;            // Operator address identifying the validator
  google.protobuf.Any new_pub_key = 3;     // New consensus public key
}
```
...
**CLI:**
```bash
simd tx poa rotate-cons-pub-key <new-pubkey-base64> <key-type> \
  --operator-address <operator-account> \
  --from <operator-or-admin> \
  -y
```

**Authorization:** Must be signed by the validator's operator address or the chain admin.
...
For the operational procedure, see [Rotate a consensus key, PoA](/sdk/next/keys/rotate-validator-key-poa).
