...
* Key rotation is not possible, although this can be solved with [account rekeying](/sdk/v0.47/build/architecture/adr-034-account-rekeying).
...
It's meant to be used with other access control modules such as [`x/feegrant`](/sdk/v0.47/build/architecture/adr-029-fee-grant-module) ans [`x/authz`](/sdk/v0.47/build/architecture/adr-030-authz-module) to simplify key management for individuals and organizations.
...
those "sub-accounts" using the [`x/authz` module](/sdk/v0.47/build/architecture/adr-030-authz-module).
...
Group accounts are stored in a `groupAccountTable`. The group account address is generated based on an auto-increment integer which is used to derive the group module `RootModuleKey` into a `DerivedModuleKey`, as stated in [ADR-033](/sdk/v0.47/build/architecture/adr-033-protobuf-inter-module-comm#modulekeys-and-moduleids). The group account is added as a new `ModuleAccount` through `x/auth`.
...
Inter-module communication introduced by [ADR-033](/sdk/v0.47/build/architecture/adr-033-protobuf-inter-module-comm) can be used to route a proposal's messages using the `DerivedModuleKey` corresponding to the proposal's group account.
