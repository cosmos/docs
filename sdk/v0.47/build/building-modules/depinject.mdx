...
[`depinject`](/sdk/v0.47/build/packages/depinject) is used to wire any module in `app.go`.
...
The App Wiring is done in `app_config.go` / `app.yaml` and `app_v2.go` and is explained in detail in the [overview of `app_v2.go`](/sdk/v0.47/build/building-apps/app-go-v2).
