...
* **Multistore:** Every application's `BaseApp` contains a [`CommitMultiStore`](/sdk/v0.47/learn/advanced/store#multistore) which is provided when a `Context` is created. Calling the `KVStore()` and `TransientStore()` methods allows modules to fetch their respective [`KVStore`](/sdk/v0.47/learn/advanced/store#base-layer-kvstores) using their unique `StoreKey`.
...
* **Transaction Bytes:** The `[]byte` representation of a transaction being processed using the context. Every transaction is processed by various parts of the Cosmos SDK and consensus engine (e.g. CometBFT) throughout its [lifecycle](/sdk/v0.47/learn/beginner/tx-lifecycle), some of which do not have any understanding of transaction types. Thus, transactions are marshaled into the generic `[]byte` type using some kind of [encoding format](/sdk/v0.47/learn/advanced/encoding) such as [Amino](/sdk/v0.47/learn/advanced/encoding).
...
* **Gas Meters:** Specifically, a [`gasMeter`](/sdk/v0.47/learn/beginner/gas-fees#main-gas-meter) for the transaction currently being processed using the context and a [`blockGasMeter`](/sdk/v0.47/learn/beginner/gas-fees#block-gas-meter) for the entire block it belongs to. Users specify how much in fees they wish to pay for the execution of their transaction; these gas meters keep track of how much [gas](/sdk/v0.47/learn/beginner/gas-fees) has been used in the transaction or block so far. If the gas meter runs out, execution halts.
* **CheckTx Mode:** A boolean value indicating whether a transaction should be processed in `CheckTx` or `DeliverTx` mode.
* **Min Gas Price:** The minimum [gas](/sdk/v0.47/learn/beginner/gas-fees) price a node is willing to take in order to include a transaction in its block. This price is a local value configured by each node individually, and should therefore **not be used in any functions used in sequences leading to state-transitions**.
...
* **Event Manager:** The event manager allows any caller with access to a `Context` to emit [`Events`](/sdk/v0.47/learn/advanced/events). Modules may define module specific
...
For example, here is a snippet from the [`runTx`](/sdk/v0.47/learn/advanced/baseapp#runtx-antehandler-runmsgs-posthandler) function in [`baseapp`](/sdk/v0.47/learn/advanced/baseapp):
...
3. If the process is running in [`checkTxMode`](/sdk/v0.47/learn/advanced/baseapp#checktx), there is no need to write the
   changes - the result is returned immediately.
4. If the process is running in [`deliverTxMode`](/sdk/v0.47/learn/advanced/baseapp#delivertx) and the result indicates
