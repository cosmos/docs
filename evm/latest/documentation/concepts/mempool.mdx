...
**LegacyPool**: A port of [Geth's transaction pool](https://github.com/ethereum/go-ethereum/blob/master/core/txpool/legacypool/legacypool.go). Stores non-executable transactions with nonce gaps, tracks dependencies, automatically promotes when gaps are filled.
...
For chain developers integrating the mempool, see the [EVM Mempool Integration Guide](/evm/latest/documentation/getting-started/build-a-chain/additional-configuration/mempool-integration).
