...
The mempool exposes Ethereum-compatible RPC methods for querying transaction pool state. See the [JSON-RPC Methods documentation](/evm/v0.4.x/api-reference/ethereum-json-rpc/methods#txpool-methods) for detailed API reference:
...
You can also explore these methods interactively using the [RPC Explorer](/evm/v0.4.x/api-reference/ethereum-json-rpc/rpc-explorer).

## Integration

<Note>
For chain developers looking to integrate the mempool into their Cosmos SDK chain, see the [EVM Mempool Integration Guide](/evm/v0.4.x/documentation/migrations/mempool-integration) for complete setup instructions.
</Note>
