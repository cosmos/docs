...
description: "Cosmos EVM can dump the entire application state to a JSON file. This, besides upgrades, can be useful for manual analysis of the state at a given height."
icon: "git-branch"
---

## Export State[​](#export-state "Direct link to Export State")
...
You can use the `migrate` command to migrate from a given version to the next one (e.g., `v0.X.X` to `v1.X.X`):
