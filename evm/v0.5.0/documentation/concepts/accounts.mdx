...
<CardGroup cols={2}>
  <Card title="Cosmos SDK Accounts" icon="circle-nodes" href="/sdk/v0.53/learn/beginner/accounts">
...
The custom Cosmos EVM EthAccount satisfies the `AccountI` interface from the Cosmos SDK auth module and includes additional fields that are required for Ethereum type addresses:
...
**EIP-7702 Support**: Cosmos EVM supports [EIP-7702 code delegation](/evm/v0.5.0/documentation/evm-compatibility/eip-7702), allowing EOAs to temporarily delegate code execution to smart contracts through the `SetCodeHash` functionality.

For more information on Ethereum accounts head over to the [x/vm module](/evm/v0.5.0/documentation/cosmos-sdk/modules/vm).
...
<Note>
For JSON-RPC methods, see [`eth_accounts`](/evm/v0.5.0/api-reference/ethereum-json-rpc/methods#eth-accounts) and [`personal_listAccounts`](/evm/v0.5.0/api-reference/ethereum-json-rpc/methods#personal-listAccounts) documentation.
</Note>
