...
<Note>
For detailed transaction flow and mempool behavior, see the [Mempool documentation](/evm/v0.5.0/documentation/concepts/mempool#transaction-flow) and [Cosmos SDK lifecycle](/sdk/v0.53/learn/beginner/tx-lifecycle).
</Note>

The transaction hash is a unique identifier and can be used to check transaction information, for example, the events emitted, and whether it was successful or not.

Transactions can fail for various reasons. For example, the provided gas or fees may be insufficient. Also, the transaction validation may fail. Each transaction has specific conditions that must be fulfilled to be considered valid. A widespread validation is that the sender is the transaction signer. In such a case, if you send a transaction where the sender address is different than the signer's address, the transaction will fail, even if the fees are sufficient.

Nowadays, transactions can not only perform state transitions on the chain in which they are submitted, but can also execute transactions on other blockchains. Interchain transactions are possible through the [Inter-Blockchain Communication protocol (IBC)](https://ibcprotocol.org/). Find a more detailed explanation in the section below.
...
This is possible because the Cosmos EVM uses the [Cosmos-SDK](/sdk/v0.53) and implements the [Ethereum Virtual Machine](https://ethereum.org/en/developers/evm/) as a module. In this way, Cosmos EVM provides the features and functionalities of Ethereum and Cosmos chains combined, and more.
...
On Cosmos chains, transactions are comprised of metadata held in contexts and `sdk.Msg`s that trigger state changes within a module through the module's Protobuf [Msg service](/sdk/v0.53/build/building-modules/msg-services).
...
<Info>
For detailed mempool behavior and flow diagrams, see [Mempool Architecture](/evm/v0.5.0/documentation/concepts/mempool#architecture).
</Info>
