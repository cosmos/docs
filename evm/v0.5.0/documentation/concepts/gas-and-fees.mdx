...
title: "Gas and Fees"
description: "Users need to pay a fee to submit transactions on a Cosmos EVM-based blockchain. As fees are handled differently on Ethereum and Cosmos, it is important to understand how a Cosmos EVM blockchain implements an Ethereum-type fee calculation, that is compatible with the Cosmos SDK."
icon: "fuel"
---
...
* [Cosmos SDK Gas](/sdk/v0.53/learn/beginner/gas-fees)
* [Ethereum Gas](https://ethereum.org/en/developers/docs/gas/)

## Basics[​](#basics "Direct link to Basics")

### Why do Transactions Need Fees?[​](#why-do-transactions-need-fees "Direct link to Why do Transactions Need Fees?")
...
### What is Gas?[​](#what-is-gas "Direct link to What is Gas?")
...
* [Cosmos Gas Fees](/sdk/v0.53/learn/beginner/gas-fees)
* [Cosmos Tx Lifecycle](/sdk/v0.53/learn/beginner/tx-lifecycle)
* [Ethereum Gas](https://ethereum.org/en/developers/docs/gas/)

### How is Gas Calculated?[​](#how-is-gas-calculated "Direct link to How is Gas Calculated?")

In general, there’s no way to know exactly how much gas a transaction will cost without simply running it. Using the Cosmos SDK, this can be done by [simulating the Tx](/sdk/v0.53/user/run-node/txs#simulating-a-transaction). Otherwise, there are ways to estimate the amount of gas a transaction will require, based on the details of the transaction fields, and data. In the case of the EVM, for example, each bytecode operation has a [corresponding amount of gas](https://ethereum.org/en/developers/evm/opcodes/).

More on Gas Calculations:

* [Estimate Gas](https://docs.ethers.org/v5/api/providers/provider/#Provider-estimateGas)
* [Executing EVM Bytecode](https://ethereum.org/en/developers/evm/opcodes/)
* [Simulate a Cosmos SDK Tx](/sdk/v0.53/user/run-node/txs#simulating-a-transaction)

### How does Gas Relate to Fees?[​](#how-does-gas-relate-to-fees "Direct link to How does Gas Relate to Fees?")
...
* [Cosmos Gas and Fees](/sdk/v0.53/learn/beginner/gas-fees)
* [Ethereum Gas and Fees](https://ethereum.org/en/developers/docs/gas/)

### How are Fees Handled on Cosmos?[​](#how-are-fees-handled-on-cosmos "Direct link to How are Fees Handled on Cosmos?")
...
At the beginning of each block, fees from the previous block are [allocated to validators and delegators](/sdk/v0.53/build/modules/distribution), after which they can be withdrawn and spent.

### How are Fees Handled on Ethereum?[​](#how-are-fees-handled-on-ethereum "Direct link to How are Fees Handled on Ethereum?")
...
### How are Gas and Fees Handled in the Cosmos EVM?[​](#how-are-gas-and-fees-handled-in-the-cosmos-evm "Direct link to How are Gas and Fees Handled in the Cosmos EVM?")

Fundamentally, a Cosmos EVM chain is a Cosmos SDK chain that enables EVM compatibility as part of a Cosmos SDK module. As a result of this architecture, all EVM transactions are ultimately encoded as Cosmos SDK transactions and update a Cosmos SDK-managed state.
...
1. Fee Market Module
   ...
   In order to support EIP-1559 gas and fee calculation on the EVM layer, Cosmos EVM tracks the gas supplied for each block and uses that to calculate a base fee for future EVM transactions, thus enabling EVM dynamic fees and transaction prioritization as specified by EIP-1559.
   ...
   Unlike on Ethereum, the `BaseFee` on a Cosmos EVM chain is not burned, and instead is distributed to validators and delegators by default, although this can be adjusted. Furthermore, the `BaseFee` is lower-bounded by the global `min-gas-price` (currently, the global `min-gas-price` parameter is set to zero, although it can be updated via Governance).

2. EVM Gas Refunds

   Cosmos EVM refunds a fraction (at least 50% by default) of the unused gas for EVM transactions to approximate the current behavior on Ethereum. This too can be adjusted.

### Detailed Timeline[​](#detailed-timeline "Direct link to Detailed Timeline")

```text "Block Processing Timeline" expandable
...
3. Nodes run `BeginBlock` for the subsequent block
   ...
   * The Distribution module [distributes](/sdk/v0.53/build/modules/distribution#begin-block) the previous block's fee rewards to validators and delegators
...
## Detailed Mechanics[​](#detailed-mechanics "Direct link to Detailed Mechanics")

### Cosmos `Gas`[​](#cosmos-gas "Direct link to cosmos-gas")
...
More information regarding gas as part of the Cosmos SDK can be found [here](/sdk/v0.53/learn/beginner/gas-fees).

### Matching EVM Gas consumption[​](#matching-evm-gas-consumption "Direct link to Matching EVM Gas consumption")

Cosmos EVM supports Ethereum Web3 tooling. For this reason, gas consumption must be equatable with other EVMs, most importantly Ethereum.
...
+++ [https://github.com/cosmos/cosmos-sdk/blob/3fd376bd5659f076a4dc79b644573299fd1ec1bf/store/types/gas.go#L187-L196](https://github.com/cosmos/cosmos-sdk/blob/3fd376bd5659f076a4dc79b644573299fd1ec1bf/store/types/gas.go#L187-L196)
...
+++ [https://github.com/cosmos/evm/blob/v0.4.1/x/vm/keeper/state\_transition.go](https://github.com/cosmos/evm/blob/v0.4.1/x/vm/keeper/state_transition.go)

### `AnteHandler`[​](#antehandler "Direct link to antehandler")

The Cosmos SDK [`AnteHandler`](/sdk/v0.53/learn/beginner/gas-fees#antehandler) performs basic checks prior to transaction execution. These checks are usually signature verification, transaction field validation, transaction fees, etc.
...
### Gas Refunds[​](#gas-refunds "Direct link to Gas Refunds")
...
### Zero-Fee Transactions[​](#zero-fee-transactions "Direct link to Zero-Fee Transactions")
...
For this same reason, in the Cosmos EVM it is possible to send transactions with `0` fees for transaction types other than the ones defined by the `evm` module. EVM module transactions cannot have `0` fees as gas is required inherently by the EVM. This check is done by the EVM transactions stateless validation (i.e `ValidateBasic`) function as well as on the custom `AnteHandler` defined by Cosmos EVM.

### Gas Estimation[​](#gas-estimation "Direct link to Gas Estimation")
...
+++ [https://github.com/cosmos/evm/blob/v0.4.1/x/vm/keeper/grpc\_query.go](https://github.com/cosmos/evm/blob/v0.4.1/x/vm/keeper/grpc_query.go)

For Cosmos Tx's, developers can use Cosmos SDK's [transaction simulation](/sdk/v0.53/user/run-node/txs#simulating-a-transaction) to create an accurate estimate.

### Cross-Chain Gas and Fees[​](#cross-chain-gas-and-fees "Direct link to Cross-Chain Gas and Fees")
...
## Dealing with gas and fees with the Cosmos EVM CLI[​](#dealing-with-gas-and-fees-with-the-cosmos-evm-cli "Direct link to Dealing with gas and fees with the Cosmos EVM CLI")
...
* `--fees=auto`: estimates fees and gas automatically (same behavior as `--gas=auto`). Throws an error if using any other fees-related flag (e.g., `--gas-prices` , `--fees`)
* `--gas=auto`: same behavior as `--fees=auto`. Throws an error if using any other fees-related flag (e.g., `--gas-prices` , `--fees`)
...
The reader should note that the former two options provide a friendlier user experience for new users, and the latter are for more advanced users, who desire more control over these parameters.

The team introduced the `auto` flag option that calculates automatically the gas and fees required to execute a transaction. In this way, new users or developers can perform transactions without the hassle of defining specific gas and fees values.
