...
* Leveraging [modules](/sdk/v0.53/build/building-modules/intro) and other mechanisms implemented by the Cosmos SDK.
* Implementing CometBFT's Application Blockchain Interface ([ABCI](/cometbft/v0.38/spec/abci/Overview)) to manage the blockchain.
* Utilizing [`geth`](https://github.com/ethereum/go-ethereum) as a library to promote code reuse and improve maintainability.
* Exposing a fully compatible Web3 [JSON-RPC](/evm/v0.5.0/api-reference/ethereum-json-rpc/methods) layer for interacting with existing Ethereum clients and tooling (MetaMask, Remix, etc.).
...
Cosmos EVM enables the full composability and modularity of the [Cosmos SDK](/). It includes standard modules from the Cosmos SDK that work side by side with EVM-specific modules. Check out the [list of modules](/evm/v0.5.0/documentation/cosmos-sdk/modules) to get an overview of what each module is responsible for.

## CometBFT & ABCI[​](#cometbft--abci "Direct link to CometBFT & ABCI")

[CometBFT](https://github.com/cometbft/cometbft) consists of two core technical components: a blockchain consensus engine and a generic application interface. The consensus engine ensures that the same transactions are recorded on every machine in the same order. The application interface, called the [Application Blockchain Interface (ABCI)](/cometbft/v0.38/spec/abci/Overview), enables transactions to be processed in any programming language.
...
* [JSON-RPC](/evm/v0.5.0/api-reference/ethereum-json-rpc) client for interacting with the EVM

Most components are implemented in the [VM module](/evm/v0.5.0/documentation/cosmos-sdk/modules/vm). However, to achieve a seamless developer experience, some components are implemented outside of the module.

To learn more about how Cosmos EVM achieves EVM compatibility as a Cosmos chain, explore the following concepts:

* [Accounts](/evm/v0.5.0/documentation/concepts/accounts)
* [Gas and Fees](/evm/v0.5.0/documentation/concepts/gas-and-fees)
* [Token representations](/evm/v0.5.0/documentation/concepts/tokens)
* [Transactions](/evm/v0.5.0/documentation/concepts/transactions)
