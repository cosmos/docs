...
For local development with `local_node.sh`, add a `jq` command after `evmd init` (alongside the other genesis patches) to inject the preinstalls before the node starts:
...
To deploy a contract beyond the defaults, add it to `NewEVMGenesisState` in your app code:
...
Then populate genesis.json before starting the node, following the same steps in [Populating genesis.json](#populating-genesisjson) above — add a `jq` entry for your custom contract alongside the defaults.
...
Both commands should return non-empty results. If they don't, the preinstall was not included in genesis.json when the chain was initialized.
